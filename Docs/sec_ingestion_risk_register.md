...
| **I08** | Concurrent writers corrupt the catalog | One designated logical writer with a lease; second writer fails loudly; WAL plus `synchronous = FULL`; explicit transactions; SQLite-consistent backups only | `unit`, `integration` |
