...
CURRENT_STAGE: M3.3 PRE-E0 — THE COMPLETE-SOURCE CANARY HAS RUN (D128) AND IS PARTIALLY ACCEPTED, WITH ITS SEMANTIC COUNTS REJECTED AND A SEMANTIC REPAIR REQUIRED (D129). Decision 129 is ACCEPTED under owner token M3_3_D129_D128_SEMANTIC_ADJUDICATION_OWNER_ACCEPTED with outcome D128_SEMANTIC_REPAIR_REQUIRED, published as a RETROSPECTIVE DURABLE RECORD of the completed complete-first-source canary m3_3_d128_complete_first_source_v1 — the Decision 129 file DID NOT EXIST BEFORE THE RUN AND DID NOT AUTHORIZE IT, and the authorizing instrument was the owner instrument issued at the time, which this repository does not hold. THERE IS NO decision_128_*.md FILE AND THERE NEVER WAS ONE: D128 NAMES A RUN IDENTITY, NOT A DECISION RECORD, the registry index steps from 127 to 129, and Decision 129 §2 records that gap as EXPECTED RATHER THAN MISSING. THE DISPOSITION IS PARTIAL AND THE PARTITION IS THE WHOLE POINT — D128 IS ACCEPTED AS AN EXECUTION AND REJECTED AS A MEASUREMENT, and neither half may be read without the other or summarized as 'D128 passed' or 'D128 failed'. ACCEPTED (Decision 129 §3, D129-R1): the full 985,834-member complete-source traversal mechanics, exactly-one-source execution mechanics, the runtime/resource evidence, bounded-memory feasibility at peak RSS 3,178,020,864 bytes = 2.9598 GiB, F1 FEASIBILITY, F2 FEASIBILITY, the Decision 127 pre-F2 admission gate's OPERATION — its first exercise in a real complete-source run, and IT ADMITTED — complete working-catalog finalization, the write-ahead-log checkpoint TO ZERO, OPERATIONAL-CATALOG NONMUTATION, the monitor/runtime evidence, and D128 ITSELF AS A VALID NEGATIVE SEMANTIC EXPERIMENT. FEASIBILITY IS NOT CORRECTNESS: every accepted item is a statement about whether the path can run, never about what it recorded. REJECTED AND NOT OWNER-CERTIFIED (Decision 129 §4, D129-R2): D128's accession counts as semantically complete, counts_are_trustworthy, parser_state as successful, D128 AS AN E0-READINESS PROOF, and ANY DOWNSTREAM RESEARCH COUNT BASED ON THE OMITTED SHARD POPULATION — NO E0 AUTHORIZATION FOLLOWS. DEFECT A, PARSER_IMPLEMENTATION_DEFECT (Decision 129 §5, D129-R3): 5,337 legitimate members matching the tracked HISTORICAL_FILE_NAME_PATTERN were routed through parse_submissions_document instead of the historical-submission contract and were rejected — THE DEFECT IS THE DISPATCH, NOT THE REJECTION — costing 5,102,087 shard accessions, of which 2,064,473 were recovered elsewhere and 3,037,614 ARE GENUINELY ABSENT from a real universe of 19,034,205, an omitted share of 15.96%, about ONE ACCESSION IN SIX; Form 10-K family 42,363 absent = 13.57% all-year, 9.11% development 2010-2021 against 0.25% evaluation 2022-2026, a differential of roughly 36x that IS RECORDED AS A STRUCTURAL CONFOUND ON THE PREREGISTERED TEMPORAL COMPARISON AND NOT AS A NEGLIGIBLE RANDOM LOSS. NO §25 DEVIATION IS RECORDED — the counts are rejected before any preregistered analysis consumed them. DEFECT B, PARSER_IMPLEMENTATION_DEFECT (Decision 129 §6, D129-R4): 5,337 of 5,337 census_historical_references rows received ONE INCORRECT OBSERVATION-WIDE REGISTRANT CIK where 4,144 distinct registrants are represented, and the compounding hazard is that _historical_registrant_cik's len(rows) != 1 guard DETECTS AMBIGUITY BUT CANNOT DETECT UNIFORM ERROR — one consistently wrong candidate passes silently and is then returned as authority. CORRECTED RESOURCE RECORD (Decision 129 §11): wall 119,923.08 s = 33 h 18 m 43 s, parse ~26 h 59 m 30 s, F1 ~1 h 44 m 47 s, F2 ~4 h 20 m 54 s, close ~7 m 27 s, post-parse ~6 h 19 m 12 s WITH A PUBLISHED 364 s RESIDUAL ATTRIBUTED TO NO NAMED COMPONENT, final working catalog 103,694,548,992 bytes = 96.5729 GiB, minimum free 18,851,307,520 bytes = 17.5568 GiB SO THE CONTINUOUS 10 GiB FLOOR WAS NEVER BREACHED, final free 28,195,401,728 bytes = 26.2589 GiB. TWO CORRECTIONS ARE PUBLISHED (Decision 129 §11.3): pre-F2 free at admission was approximately 31.98-32.01 GiB AND NOT approximately 37 GiB, so THE REAL MARGIN ABOVE THE 30 GiB GATE WAS APPROXIMATELY 2 GiB; and the approximately 15.17 GiB PEAK WAL OCCURRED BEFORE THE PRE-F2 GATE during parse-finalization/resolution, while F2's OWN WAL PEAK WAS APPROXIMATELY 8.67 GiB. Actual F1+F2 6 h 05 m 41 s against the Decision 124 §8 planning ~3.37 h is an UNDERPREDICTION OF ~1.8x-1.9x. MIGRATION HEAD REMAINS 0015 AND 0016 IS ABSENT AND UNAPPLIED. ALL THREE ACTIVATION CONSTANTS REMAIN None. E0 REMAINS UNAUTHORIZED. NO CORRECTED RERUN IS AUTHORIZED. D129 SUPERSEDES NOTHING — Decisions 121 through 127 stand as written and EVERY D124-R5 GATE CARRIES FORWARD INTACT, INCLUDING the >= 30 GiB pre-F2 gate, which §11.3 shows was met by about 2 GiB — A CALIBRATION FACT, NOT A RELAXATION.
ACTIVE_BLOCKER: TWO BLOCKING PARSER IMPLEMENTATION DEFECTS STAND BETWEEN THIS REPOSITORY AND ANY E0 AUTHORIZATION, AND NEITHER IS REPAIRED — (1) BULK HISTORICAL-SHARD DISPATCH (D129-R3) and (2) PER-REFERENCE PARENT REGISTRANT BINDING (D129-R4). THREE FURTHER PREREQUISITES STAND BEFORE ANY CORRECTED RERUN: (3) WATCHDOG SIGINT DELIVERY REPAIR (D129-R10), (4) POST-TRAVERSAL MONITORING CORRECTION (D129-R11), and (5) A CORRECTED-RUN CAPACITY RECONCILIATION (D129-R12). THE FORMER D127 REVIEW-AND-ACCEPTANCE BLOCKER IS OVERTAKEN BY EVENTS RATHER THAN DISCHARGED HERE: the guard has now been exercised in a real complete-source run and Decision 129 §3 (D129-R1) ACCEPTS ITS OPERATION, while the D127 acceptance and the D128 launch authorization were OWNER ACTS ISSUED OUTSIDE THIS REPOSITORY that this repository holds no record of beyond what Decision 129 now publishes. THE CAPACITY BLOCKER IS BACK, IN A NEW FORM: Decision 129 §12 (D129-R12) rules the old Decision 124 launch-capacity model NOT SUFFICIENT BY ITSELF to authorize a corrected rerun, on three independent grounds — actual storage and runtime EXCEEDED important D124 projections at an F1+F2 underprediction of ~1.8x-1.9x, the real pre-F2 margin was only ~2 GiB, and A CORRECTED PARSE ADDS MILLIONS OF PRESENTLY ABSENT ACCESSIONS AND RECORDS so D128's 96.5729 GiB catalog is a FLOOR AND NOT AN ESTIMATE. NO D124-R5 GATE IS RELAXED BY THAT RULING and Decision 129 CONSTRUCTS NO NEW MODEL. D128 IS NOT RESUMABLE AND NOT REPAIRABLE IN PLACE (D129-R8): a corrected complete-source proof requires NEW CODE, A NEW RUN IDENTITY, A NEW CREATE-ONCE WORLD, AND A FULL SOURCE RERUN FROM THE BEGINNING. D128 IS RETAINED UNCHANGED as valuable pre-correction evidence and MUST NOT BE ARCHIVED, DELETED, MOVED, OR MUTATED BEFORE D130. Nothing in this repository is an execution authorization: not this file, not a passing gate, not a commit, not a push, and not the Decision 129 record — ACCEPTING A RUN'S MECHANICS IS NOT AUTHORIZING ANOTHER RUN. D118 R23 still binds. All three execution authorities are None, migration 0016 is unapplied and the operational catalog stays at head 0015, no E0-v3 namespace exists, and both tracked network switches stay false at request ceiling 0.
IMPLEMENTATION_AUTHORIZATION: EXERCISED FOR ONE GOVERNANCE PUBLICATION AND NOTHING ELSE — THE OWNER'S 2026-08-22 DECISION 129 D128 SEMANTIC ADJUDICATION OWNER-ACCEPTANCE / GOVERNANCE PUBLICATION INSTRUCTION, TOKEN M3_3_D129_D128_SEMANTIC_ADJUDICATION_OWNER_ACCEPTED, AUTHORIZED THE DOCUMENTATION-ONLY D129 PUBLICATION by expressly directing the creation of Decision 129 and the update of the decision registry, the decision index, and STATUS. THE INSTRUMENT'S PROHIBITIONS WERE HONOURED IN FULL: NO PRODUCTION CODE EDITED, NO TESTS EDITED, NO PARSER BEHAVIOUR REPAIRED, D128 NOT RERUN, NO NEW CANARY, NO E0, NETWORK NOT ENABLED, MIGRATION 0016 NOT APPLIED, D128 NOT MUTATED, D128 NOT ARCHIVED OR DELETED OR MOVED, OPERATIONAL EVIDENCE NOT MODIFIED, AND NO AUTHORITY CONSTANT CHANGED. The touched paths are Docs/Decisions/decision_129_m3_3_d128_semantic_adjudication.md (new), one new Docs/Decisions/decision_registry.md index row plus one new open-items bullet recording the absent decision 128 file, eighteen new Docs/decision_index.md navigation rows, and this file. Docs/change_impact_map.md IS DELIBERATELY UNTOUCHED — repository convention updates it only when a code surface changes, as it was for D127 and was not for D120 through D126, and D129 CHANGES NO CODE SURFACE. src/disclosure_drift/m3/capacity_plan.py IS UNTOUCHED and no Decision 113 capacity constant moved. THE D128 RUN ITSELF WAS PERFORMED BEFORE AND OUTSIDE THIS PUBLICATION, under the owner instrument issued at the time; THIS SESSION EXECUTED NO GOVERNED OPERATION: no parse, no F1, no F2, no canary, no disposable world, no complete-source run, no E0, no network, no migration 0016, no governed SQLite writer, and no catalog, source, or evidence mutation. Read-only verification was performed and is reported: git entry state, migration head, the three authority constants, both network switches, and a metadata-only inventory of the retained D128 world that OPENED NO DATABASE — the working catalog reads 103,694,548,992 bytes with a 0-byte write-ahead log, matching the accepted D129 §11.2 figure exactly, and the operational catalog reads 359,378,944 bytes with a 0-byte write-ahead log and an mtime PREDATING the D128 launch, corroborating the accepted operational-catalog nonmutation. NO OWNER-ACCEPTANCE TOKEN WAS EMITTED BY THIS SESSION.
NEXT_AUTHORIZED_ACTION: D130 — THE D128 EXTERNAL ARCHIVAL AND INTERNAL RECLAMATION — AND NOTHING ELSE. IT IS NOT EXECUTED AND IS NOT AUTHORIZED BY ANYTHING WRITTEN HERE; this entry records OWNER SEQUENCING ONLY and D130 REQUIRES ITS OWN OWNER INSTRUMENT. UNTIL THEN D128 IS RETAINED EXACTLY AS THE RUN LEFT IT (D129-R8) and may not be archived, deleted, moved, or mutated. THE FULL SEQUENCE AFTER D130, EACH STEP REQUIRING ITS OWN OWNER INSTRUMENT: a BOUNDED parser, reference, and watchdog correction stage implementing the Decision 129 §7 identity and dispatch invariants (D129-R5: the explicit parent filings.files[].name declaration is AUTHORITATIVE, a filename CIK is CORROBORATIVE ONLY AND NEVER THE SOLE INFERENCE RULE, and no parent / multiple parents / conflicting parent identities / parent-versus-member-name contradiction all FAIL CLOSED; D129-R6: ARCHIVE MEMBER ORDER MAY NOT AFFECT CORRECTNESS, shard-before-parent must be tolerated BOUNDEDLY AND DETERMINISTICALLY, and NO ALGORITHM IS FROZEN beyond those invariants), the Decision 129 §9 watchdog repair (D129-R10: SIGINT DELIVERY MUST BE POSITIVELY PROVEN and a watchdog MAY NOT TREAT A SUCCESSFUL kill(2) RETURN AS PROOF THAT THE PROCESS STOPPED), and the Decision 129 §10 monitoring correction (D129-R11: member-count stall detection applies ONLY WHILE member_count < governed_member_count or must use a phase-appropriate signal); THEN semantic validation; THEN the corrected-run capacity reconciliation (D129-R12); THEN the corrected complete-source canary, which must be a FULL RERUN FROM SCRATCH IN A NEW WORLD. Decision 129 §8 (D129-R7) accepts lei, filings.recent.core_type and filings.recent.isXBRLNumeric as OPTIONAL NON-SEMANTIC SEC FIELDS registrable later, WITH NO CODE CHANGE NOW. D129-R9 preserves D117, D120, D122 and D123 for their original purposes — they STRUCTURALLY COULD NOT have reached terminal parser semantic adjudication, and NO PRIOR OWNER ACCEPTANCE IS RETROACTIVELY INVALIDATED. THE FOUR NON-CODE D124-R5 REQUIREMENTS REMAIN THE LAUNCH WRAPPER'S TO ENFORCE (D126-R5), THE >= 30 GiB PRE-F2 GATE REMAINS IN CODE, NO FURTHER DISCLOSURE DRIFT EVIDENCE MAY BE DELETED FOR CAPACITY (D125-R3), THE SSK SSD REMAINS COLD/ARCHIVE ONLY WITH NO REFORMAT (D125-R4), AND PLAN B REMAINS NOT APPROVED. COMPLETE SOURCE IS NOT AUTHORIZED AND E0 IS NOT AUTHORIZED. Do not emit an owner-acceptance token — this session may publish an accepted ruling and may not accept anything.
