...
CURRENT_STAGE: M3.3 — THE D139 FABLE REVIEW IS OWNER ACCEPTED AND EVERY ACTIONABLE FINDING IS CORRECTED AND PUBLISHED AS DECISION 140; NEITHER D137, D138 NOR D140 IS OWNER ACCEPTED AND NONE AUTHORIZES A CANARY. THE PRIOR CANARY AUTHORIZATION IS SUPERSEDED AND WITHDRAWN — CANARY_AUTHORIZED = NO. The accepted verdict is D139_FABLE_REVIEW_CORRECTION_REQUIRED — 0 BLOCKER / 2 MAJOR / 7 MINOR / 10 INFORMATIONAL — under review-acceptance token M3_3_D139_FABLE_REVIEW_FINDINGS_OWNER_ACCEPTED and correction authorization M3_3_D140_TOTAL_PRE_CANARY_HARDENING_AUTHORIZED (issued outside this repository, now spent), completion token M3_3_D140_TOTAL_PRE_CANARY_HARDENING_COMPLETE_READY_FOR_FABLE_REVIEW, AND NO ACCEPTANCE TOKEN. DECISIONS 137, 138 AND 139 REMAIN HISTORICAL AND ARE NOT REWRITTEN. MAJOR-1 — THE ABSENT AND STALE MOUNT REACHED AN INTERNAL FALLBACK: D138 decided external-or-internal by DEVICE NUMBER ON THE NEAREST EXISTING ANCESTOR, which has a wrong answer whenever the volume is not there — with the SSD unplugged /Volumes/SSK SSD/world resolves to /Volumes, and a WRITABLE DIRECTORY LEFT AT THE MOUNT POINT resolves to itself; BOTH CLASSIFY INTERNAL AND NO GUARD RUNS AT ALL. INTENT IS NOW READ FROM THE NAME (D140-R1): a path under /Volumes/<name>/ is an external intent WHETHER OR NOT ANYTHING IS MOUNTED and NEVER DEGRADES TO INTERNAL. --require-volume-uuid IS MANDATORY (D140-R2) and omission is the refusal. THE DIRECTORY MUST EXIST, BE CURRENTLY A MOUNT POINT, AND REPORT EXACTLY 397A4D4A-9508-391E-814E-3B533C7BD049 (D140-R3). THE VOLUME IS RE-AUTHENTICATED AT THE LAST SAFE POINT BEFORE mkdir (D140-R4). create_world CREATES NO PARENT ON THE EXTERNAL PATH (D140-R5) — the accepted mkdir(parents=True) would have RECREATED THE MOUNT POINT on the internal disk and built the world inside it, blocked only by /Volumes being root-owned. AdmittedVolume PINS MOUNT POINT AND DEVICE NUMBER (D140-R15). MAJOR-2 — ONE CANONICAL DURABLE LAUNCH ENVELOPE (D140-R6, D140-R10): runbook §28e is the single command and every other snippet is marked superseded — tmux -e SQLITE_TMPDIR because an EXPORTED value reaches a pane only on a NEW SERVER and attaching to a running one spills to the internal volume SILENTLY FOR THIRTY HOURS; durable stdout/stderr by dup2 BEFORE exec; caffeinate -dims held for the child lifetime; /usr/bin/time -l -o for PEAK RSS; and an INTERNAL pid record never derived from WORK_ROOT. caffeinate and time are ANCESTORS, not exec targets. MINOR-1: SQLite UNLINKS etilqs_* immediately so a directory walk returns zero WHILE A SPILL IS IN PROGRESS — temp_bytes IS NOW ALWAYS null beside UNMEASURED_UNLINKED_SQLITE_TEMP, with the visible walk renamed temp_visible_bytes; FREE SPACE REMAINS AUTHORITATIVE. MINOR-2 (OWNER-UPGRADED TO CANARY-CRITICAL): A BLOCKING F0 NOW STOPS — F1 0 calls, F2 0 calls, NO RESULT DOCUMENT, gate-failure exit, world untouched, nothing retried. MINOR-3: the accepted post-F0 test PROVED NOTHING because the tuple assignment never executed; it is STRENGTHENED, NOT DELETED. MINOR-4/5/6/7: runtime-control paths refused inside the work root or D130; the SQLITE_TMPDIR refusal now names the TEMPORARY ROOT; ONE HOST-LEVEL fcntl.flock INDEPENDENT OF run_id, which is NOT a second writer-lease architecture; and the source is AUTHENTICATED BEFORE create_world with F0's own verification NOT weakened. INFO-1/2: between PRE_LAUNCH and POST_F0 the accepted run took NO CAPACITY READING AT ALL across ~985,000 parts — sampling now sits at F0's per-part and finalization boundaries, F1's per-accession boundary, and BRACKETS F2's totality tail. INFO-3: free space read after the volume has gone DESCRIBES THE INTERNAL DISK, WHICH ALWAYS LOOKS HEALTHY — every sample re-establishes containment and pinned st_dev first and loss raises F2_VOLUME_IDENTITY_LOST fail-closed. INFO-5: alert evidence bounded IN MEMORY at <=240 records. INFO-6: stop-canary reads the EXACT PID and SCANS NOTHING, refusing a shell on argv[0]. INFO-7 is RESOLVED_BY_RECOVERY_CONTROL and INFO-8 is RESOLVED_BY_CONTROL — the physical facts are NOT claimed to have vanished. INFO-9/10: a READ-ONLY STRUCTURAL PREFLIGHT now proves the D129 parent map BEFORE F0, and ON THE REAL SOURCE IT RETURNED 985,834 MEMBERS, 5,337 SHARDS, 5,337 DECLARED, 0 ORPHAN, 0 DUPLICATE-PARENT, 0 CONFLICTING-PARENT, PARENT MAP SOUND IN 108.432 s. VALIDATION: focused baseline 270; FULL SUITE 5107 PASSED / 1 PRE-EXISTING SKIPPED / 0 FAILED, exactly +75; 22 REVERSIBLE MUTATIONS, ALL 22 KILLED, every file byte-identical by SHA-256 afterwards, NO SOURCE-MUTATING FRAMEWORK. ONE TEST WEAKNESS WAS FOUND BY FALSIFICATION AND IS RECORDED. A CI FAILURE ON THE FIRST PUSH IS ALSO RECORDED RATHER THAN TIDIED AWAY: run 32658404416 failed its required SEC-ENABLED job at 1 FAILED / 5105 PASSED / 2 SKIPPED because test_m2_the_resource_report_is_captured_durably invoked /usr/bin/time -l, a BSD FLAG GNU time REJECTS ON A LINUX RUNNER. THE DEFECT WAS IN THE TEST, NOT IN PRODUCTION CODE — nothing under src/ invokes /usr/bin/time — and it is repaired on the D133 PRECEDENT by a CAPABILITY GUARD that skips where the flag is unsupported, with the guard itself proved to SKIP rather than pass vacuously. ONE BOUNDED READ-ONLY LIVE PREFLIGHT RAN: UUID EXACT; archive 24 ENTRIES, NO DIFFERENCES with the 103,966,696,960-BYTE TAR STAT-ED AND NEVER OPENED; free 310,498,426,880 BYTES / 289.1742 GiB clearing 185/60/55; source bytes and SHA-256 both EXACT; FIVE LIVE REFUSAL CONTROLS ALL REFUSED and a LIVE SUBSTRING DECOY WAS REFUSED AND LEFT ALIVE; NOTHING WAS CREATED, WRITTEN OR DELETED ON THE VOLUME. GOVERNED PAUSE/RESUME IS NOT IMPLEMENTED and returns D140_PAUSE_RESUME_ARCHITECTURE_REQUIRES_OWNER_REDESIGN under A1-R20; the SOUND SUBSET is identified and offered. NO MIGRATION: HEAD REMAINS 0015 AND 0016 IS ABSENT. ALL THREE ACTIVATION CONSTANTS REMAIN None. NETWORK REMAINS DISABLED at both tracked switches. census_orchestrator.py::_parse_bulk REMAINS AN EXPLICIT PRE-NETWORK BLOCKER, DELIBERATELY UNREPAIRED. A COMPLETE-SOURCE CANARY IS NOT AUTHORIZED. E0 IS NOT AUTHORIZED.
ACTIVE_BLOCKER: THE CORRECTED COMPLETE-SOURCE CANARY REMAINS UNAUTHORIZED, AND THE PRIOR AUTHORIZATION IS WITHDRAWN. D140's correction is COMPLETE but NOT INDEPENDENTLY REVIEWED AND NOT OWNER ACCEPTED, and neither D137 nor D138 was ever owner accepted; the D136-R8 narrow one-canary exception takes effect ONLY AFTER IMPLEMENTATION AND ACCEPTANCE, and correction alone moves nothing. A passing preflight still prints canary_authorized: false. SEPARATELY, GOVERNED PAUSE/RESUME IS AN OPEN OWNER-REDESIGN ITEM (Decision 140 §17): there is NO SAFE_TO_EJECT STATE, kill -STOP IS NOT A GOVERNED PAUSE, AND THE SSD MUST NOT BE DISCONNECTED WHILE A CANARY RUNS. NINE RESIDUAL LIMITATIONS ARE CARRIED RATHER THAN CLOSED (Decision 140 §18), INCLUDING: macos_volume_identity IS macOS-ONLY; THE RESIDENCE CLASSIFIER IS A MOUNT-POINT TEST, NOT A DEVICE-CLASS TEST; DURING_F2 SAMPLING IS BOUNDED, NOT INSTANTANEOUS; THE EXACT-UUID RE-READ RUNS ON A BOUNDED INTERVAL; SQLITE_TMPDIR PEAK SPILL REMAINS UNMEASURED; NO LIVE END-TO-END REHEARSAL WAS PERFORMED; FULL-SCALE PEAK RSS CANNOT BE MEASURED UNTIL A REAL RUN; THE LID MUST PHYSICALLY REMAIN OPEN AND ExFAT STILL HAS NO JOURNAL; AND A FAILED LATE WORLD STILL CANNOT BE DISPOSED OF WITHOUT ITS OWN OWNER AUTHORIZATION. SEPARATELY AND INDEPENDENTLY, census_orchestrator.py::_parse_bulk REMAINS AN OPEN PRE-NETWORK BLOCKER whose repair MUST NOT be performed as a side effect of unrelated work.
IMPLEMENTATION_AUTHORIZATION: SPENT — M3_3_D140_TOTAL_PRE_CANARY_HARDENING_AUTHORIZED was issued outside this repository and is now consumed by this correction. NO FURTHER IMPLEMENTATION AUTHORITY ARISES FROM THE DECISION 140 PUBLICATION. It does NOT accept Decision 137, 138 or itself, does NOT adopt the external volume, does NOT create a canary world, a run identity, an execution namespace, a launch receipt, or a terminal state, does NOT authorize a canary — THE PRIOR CANARY AUTHORIZATION IS SUPERSEDED AND WITHDRAWN AND NO REPLACEMENT IS ISSUED — does NOT authorize E0, does NOT create or authorize migration 0016, does NOT authorize network, SEC or HTTP at request ceiling 0, does NOT repair census_orchestrator.py::_parse_bulk, does NOT change D131 parser or runtime semantics, does NOT adopt any D134 candidate, does NOT widen the D136-R8 one-canary exception, and does NOT implement governed pause/resume. IT CLAIMS NO OWNER ACCEPTANCE.
NEXT_AUTHORIZED_ACTION: ONE FRESH INDEPENDENT REREVIEW OF THE DECISION 140 CORRECTED TARGET — AND NOTHING ELSE. It must require 0 BLOCKER / 0 MAJOR / 0 MINOR / 0 ACTIONABLE INFORMATIONAL before the owner considers issuing a new complete-source canary authority. It is not started and must not be started from this publication. A session that creates a canary world, a run identity, an execution namespace or a launch receipt, that writes to the SSK SSD, that sets any activation constant, that adds migration 0016, or that enables network on the strength of D137, D138 or D140 is acting OUTSIDE all three. THE SSD MUST NOT BE WRITTEN TO under this publication, the D130 archive MUST NOT BE MODIFIED, MOVED, DELETED OR HASHED beyond its four compact proofs, and the large D130 tar MUST NOT BE OPENED. After review, an OWNER DECISION ON WHETHER TO ACCEPT DECISIONS 137, 138 AND 140 — and, separately and only then, whether to authorize the corrected canary, and whether to commission the Decision 140 §17 pause/resume redesign.
...
NEXT_AUTHORIZED_ACTION: EXECUTE M3.3-E0 UNDER THE ALREADY-ACCEPTED FROZEN E0 SCOPE, IN A GENUINELY NEW CLAUDE OPUS 5 MAXIMUM SESSION WITH ACTUAL-MODEL ATTESTATION BEFORE SUBSTANTIVE WORK — AND NOTHING BEYOND IT. Authority: accepted Decision 092 section 11 (M3_3_E0_OWNER_AUTHORIZED) as refined by accepted Decision 093 sections 6-11. E0 runs ONLY under its already-accepted frozen M3.3 scope with NO methodology broadening, NO new SEC request, and NO network, using accepted stored M3.2 source objects only, writing ONLY the R17 fifteen-table footprint plus the census_plan_sources.parser_state transition into the accepted real private catalog, and observing the Decision 093 section 10 execution invariants (project Python 3.12, not system 3.9; storage.sqlite.connect() through its context-manager API; the private root resolved once per process and cached; the order compute -> validate -> independently recompute every identity from the persisted rows -> verify integrity -> THEN freeze; NO persisted digest computed from a preimage containing its own value; per-column private-path validation). AFTER a successful E0 freeze: run the already-authorized READ-ONLY linkage-resolution diagnostic over exactly the 96 owner-accepted form+date D091 assertions under M3_3_R52_LINKAGE_RESOLVER_USES_ASSOCIATION_SET_PLUS_STATED_FILING_DATE — complete established registrant association set (never the nullable anchor for a multi-registrant accession), original form EXACTLY 10-K or 10-KT, candidate original filing_date EXACTLY equal to the issuer-stated original_filing_date with report_date NOT the matching field, union, dedupe by canonical original accession, classify ZERO / EXACTLY_ONE / MULTIPLE / UNESTABLISHED_ASSOCIATION_SET, with the 6 form-only partials outside the denominator as NO_DATE / INELIGIBLE_FOR_LINKAGE_RESOLUTION, and acceptance ordering evaluated separately as ORDERING_PASS / ORDERING_FAIL / ORDERING_UNAVAILABLE where missing evidence is NEVER read as PASS — persisting NO verified linkage evidence and granting NO quota credit. THEN RETURN TO SOL/GPT for linkage-gate adjudication; only Sol may close M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE. DO NOT automatically start the single-pass owner-adjudication persistence bridge, migration 0016, M3.3-E1, M3.3-E2, or M3.4. NETWORK, SEC, AND HTTP AUTHORITY IS NONE at REQUEST_CEILING 0 with new SEC requests 0. Detailed authority remains in accepted Decisions 092 and 093
