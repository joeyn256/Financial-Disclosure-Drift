...
IMPLEMENTATION_AUTHORIZATION: THE VERIFIED-EVIDENCE SCHEMA AND MIGRATION 0015 — AND NOTHING ELSE (accepted Decision 087 section 4 and section 13; contract at accepted Decision 082 section 11 as refined by accepted Decision 083 R63). MIGRATION_AUTHORIZED IS 0015 ONLY, kept SEPARATE from 0014, which is neither rewritten nor squashed, and ordered after it. The four relations document_artifacts, document_review_records, document_review_spans, and document_adjudicated_evidence are created EMPTY as infrastructure for FUTURE reviewed document evidence and are populated only by synthetic disposable test fixtures. document_artifacts is CATALOG METADATA ONLY: Complete Submission Text bytes stay in the private external evidence root and never enter SQLite, and NO absolute EV_ROOT path, private filesystem path, local user path, or scratch path is persisted, with no locator that could expose one. Review records MECHANICALLY DISTINGUISH Review A, Review B, and adjudication through durable OPAQUE review-epoch identifiers plus reviewer role and model, with NO personal name and NO raw Claude session ID required, and may store the frozen future protocol version m3.3-document-evidence/1.0 exactly as contracted WITHOUT executing it. Spans carry exact source-span provenance with NO classifier and NO automated evidence-inference system invented and IN-2 unreversed. evidence_level = verified is authorized ONLY for amendment purpose and amendment linkage / explicit-original evidence, ENFORCED by schema and policy validation and never silently enabled for size, industry, history, universe, cohort, XBRL eligibility, control predicates, name/ticker, or any other dimension. NO new semantic state such as verified_amends_original is invented: amendment_linkage_state = amends_original is REUSED and verification strength lives separately in evidence_level = verified with its document/review/adjudication provenance. Existing evidence-level validation for every other dimension is NOT weakened and existing synthetic/rehearsal rows are NOT silently reinterpreted. The relations follow the accepted append-only, freeze-oriented model and refuse in-place mutation of a frozen review, rewritten source-span provenance, a changed artifact SHA after review, a changed reviewer role/epoch after freeze, and a changed adjudicated result after final freeze, with NO delete/update flexibility invented for convenience. Hash discipline: NEW evidence-specific domains through the existing release/hashing.py with no second hash implementation; ACCESSION_TABLE_COLUMNS, REGISTRANT_TABLE_COLUMNS, and SNAPSHOT_CONTENT_FIELDS are UNTOUCHED per accepted R67; no historical synthetic identity is altered; and candidate identity does NOT change merely because 0015 exists — the required proof being that a database upgraded from 0014 to EMPTY 0015 evidence tables preserves every pre-existing candidate/selection identity output except the already-accepted R68 migration-chain policy binding, whose movement must be enumerated explicitly and distinguished from evidence-content movement, with STOP rather than widening an owner-frozen candidate identity tuple. Authorized paths are exactly the accepted Decision 087 section 13 list: src/disclosure_drift/storage/migrations/0015_m33_verified_document_evidence.sql, src/disclosure_drift/m3/document_evidence.py, the src/disclosure_drift/m3/__init__.py re-export, FINAL_MIGRATION_VERSION 14 -> 15 in src/disclosure_drift/m3/acquisition.py (that constant and NOTHING ELSE in that file, on the identical owner interpretation accepted for R65), the new and existing unit tests covering the verified-evidence schema, migration provenance, the storage catalog chain, the R46 chain-head expectations and the section 9 migration-chain re-baseline, Docs/sec_data_dictionary.md, Docs/architecture_map.md, Docs/change_impact_map.md, Docs/decision_index.md, Docs/Decisions/decision_registry.md, and Milestones/STATUS.md. Prohibited: cohorts.py, pilot_policy.py, migrations 0001-0014, preregistration.md, every existing Docs/Decisions record, every network/acquisition/transport module, the R46 implementation, the D081 private verification evidence, the accepted M3.2 evidence root, candidate-selection methodology, document-classification logic, real review execution, and E0 execution. NO document review executes: Review A, Review B, adjudication, purpose classification, linkage adjudication, and witness counting are all NOT performed, the 108 real D081 review outcomes are NOT inserted, and the D081 private evidence artifacts are NOT accessed. Migration 0015 is PROSPECTIVE, non-destructive, compatible with empty verified-evidence state, and safe on a disposable catalog through 0014; any requirement to reinterpret or destroy accepted real evidence is a STOP. Fourteen adversarial protections VE-M1 through VE-M14 must be DEMONSTRATED rather than named. M3.3-E0, M3.3-E1, M3.3-E2, and M3.4 all remain UNAUTHORIZED, network, SEC, and HTTP authority is NONE with REQUEST_CEILING 0, and IMPLEMENTATION IS NOT ACCEPTANCE — migration 0015 needs a fresh independent review AND Sol/GPT owner acceptance before real document-review execution begins
ACTIVE_STAGE_CONTRACT: Milestones/contracts/m3_3.md
NEXT_AUTHORIZED_ACTION: RETURN TO SOL/GPT FOR A FRESH INDEPENDENT REVIEW OF THE MIGRATION-0015 VERIFIED-EVIDENCE IMPLEMENTATION — AND NOTHING ELSE. The accepted Decision 087 implementation stage is COMPLETE: migration 0015_m33_verified_document_evidence.sql creates the four Decision 082 section 11.2 relations (document_artifacts, document_review_records, document_review_spans, document_adjudicated_evidence) EMPTY, adds src/disclosure_drift/m3/document_evidence.py with the frozen vocabularies, the verified-applicability gate, the private-path validator, and seven NEW evidence-specific hash domains through the existing release/hashing.py, widens EXACTLY TWO constraints on pilot_candidate_accessions (amendment_purpose_evidence_level admits 'verified'; amendment_purpose_quota_eligible admits 'provisional' OR 'verified') and no others, and moves FINAL_MIGRATION_VERSION 14 -> 15 in acquisition.py, that constant and nothing else in that file. PROVED: fresh-build and 0014->0015 upgrade schemas byte-identical over 371 objects; foreign_key_check and integrity_check clean on both paths; the prospective empty-state guard refuses a populated catalog and leaves it at 0014 with no evidence table created and its rows intact; migrations 0001-0014 byte-unchanged and provenance recognizing 0015's final bytes; VE-M1 through VE-M14 all EFFECTIVE with all NINETEEN guards independently DEMONSTRATED load-bearing by removal; a positive control proving the lawful protocol lifecycle is still admitted; and IDENTITY NONCHANGE — no frozen tuple widened, no candidate column added, the pinned pre-0015 candidate-accession digest reproducing, and the ONLY movement the accepted R68 migration-chain policy binding (selector_policy_sha256, root_manifest_sha256, manifest_id, and the canonical-JSON length 275547 -> 275721 for one added block-5 row), with candidate_tables_sha256 and selection_result_sha256 BYTE-IDENTICAL and NO evidence content existing anywhere. ONE routine make check-fast returned exit 0. IMPLEMENTATION IS NOT ACCEPTANCE: migration 0015 requires a FRESH INDEPENDENT REVIEW and SOL/GPT OWNER ACCEPTANCE before real document-review execution begins. UNTIL THAT LANDS: DOCUMENT REVIEW A, DOCUMENT REVIEW B, AND THE DOCUMENT ADJUDICATION REMAIN UNAUTHORIZED; M3.3-E0, M3.3-E1, M3.3-E2, AND M3.4 REMAIN UNAUTHORIZED; the 108 real D081 review outcomes are NOT inserted and the D081 private evidence was NOT accessed; M3_3_REAL_AMENDMENT_PURPOSE_FEASIBILITY_GATE_OPEN and M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE_OPEN both remain ACTIVE and are never merged into one flag; M3_3_REAL_ACCEPTANCE_ORDERING_ADEQUACY is PENDING FUTURE AUTHORIZED E0 VERIFICATION; NETWORK, SEC, AND HTTP AUTHORITY IS NONE WITH REQUEST_CEILING 0; MIGRATION_AUTHORIZED is NONE (the 0015 stage is complete and committed) with 0016 ABSENT; and m3.2-complete remains unmoved with NO tag created. The implementing session did NOT self-review
...
M3_3_VERIFIED_EVIDENCE_SCHEMA_IMPLEMENTATION_STATUS: IMPLEMENTED AND VALIDATED, PENDING FRESH INDEPENDENT REVIEW AND OWNER ACCEPTANCE — authorized by accepted Decision 087 section 4 and section 13; contract at accepted Decision 082 section 11 as refined by accepted Decision 083 R63. Migration chain is now 0001-0015 contiguous with 0001-0014 BYTE-UNCHANGED. The four relations ship EMPTY and are exercised only by synthetic disposable fixtures under pytest tmp_path: NO real D081 evidence is inserted and the D081 private evidence artifacts were NOT accessed. document_artifacts persists NO absolute EV_ROOT path, private filesystem path, local user path, or scratch path and has NO locator column at all, because artifact_sha256 is itself the content address. Review epochs are durable OPAQUE 64-hex identifiers plus reviewer role and model, with UNIQUE (accession_plain, reviewer_role) and a one-role-per-epoch trigger together forcing three distinct epochs per accession while still permitting one epoch to review all 108 artifacts. evidence_level = 'verified' is admitted ONLY for amendment purpose and amendment linkage / explicit-original, enforced at the evidence_kind CHECK and by document_evidence.require_verified_evidence_applicable, and a verified candidate level additionally requires a frozen adjudicated row to stand behind it. NO verified_amends_original state exists anywhere as a value. All four relations are append-only and frozen at insert, with UPDATE and DELETE refused outright and the append-after-consumption doors closed. The stage touched NO research definition, quota, selector, cohort, seed, cohorts.py, pilot_policy.py, candidate_identity.py, network/acquisition/transport module, or migration 0001-0014. NEXT: fresh independent review, then Sol/GPT owner acceptance. REVIEW A, REVIEW B, THE DOCUMENT ADJUDICATION, E0, E1, E2, AND M3.4 ALL REMAIN UNAUTHORIZED
