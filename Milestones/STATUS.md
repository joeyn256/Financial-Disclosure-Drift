...
independently reviewed, accepted, and published in that order; combined stage T2.5–T2.6 remains
owner-gated and not begun.** **The next authorized action is
`CHATGPT_OWNER_ISSUANCE_OF_M3_2_G1_IMPLEMENTATION_PACKET_AFTER_DECISION_043_PUBLICATION`** —
accepted
[Decision 043](../Docs/Decisions/decision_043_m3_2_g1_navigation_workflow_repair_authorization.md)
(2026-08-06, outcome `M3_2_G1_NAVIGATION_AND_WORKFLOW_REPAIR_AUTHORIZED`) authorizes the bounded
**non-production** stage **M3.2 G1 — Navigation and Workflow Repair** on a seven-path ceiling, and
**G1 has not begun**: it may not begin before the separate owner-issued G1 implementation packet,
and it changes no production source or test behaviour. This
file records
...
**`CHATGPT_OWNER_ISSUANCE_OF_M3_2_G1_IMPLEMENTATION_PACKET_AFTER_DECISION_043_PUBLICATION`** —
control returns to the ChatGPT owner to issue the bounded G1 implementation packet. **G1 has not
begun and may not begin from this marker alone.**
...
**Combined stage T2.5–T2.6 remains owner-gated, unauthorized, and not begun.** Neither T2.4
acceptance nor Decision 043 authorizes it: T2.5–T2.6 requires its own separate explicit owner stage
authorization, and nothing in this ledger authorizes a session to begin T2.5 or T2.6 from this
marker alone.
...
intentional skip** (the long-standing `[sec]`-extra skip), and static gates clean. The rereview
...
Milestone 3.2 T3 implementation acceptance, which has NOT occurred**. Combined stage T2.5–T2.6
remains owner-gated and has not begun, and its commit remains the implementation-freeze candidate
for the independent T3 review; T3, T4, and each per-window T5 remain separate later owner acts.
...
T2.5–T2.6 remained owner-gated, unauthorized, and not begun at that acceptance** (stage T2.4 has
since been authorized by accepted Decision 040 and, after the Decision 041 correction and a passing
fresh independent rereview, **accepted and published** by accepted Decision 042, 2026-08-06 — see
the marker block above; combined T2.5–T2.6 remains owner-gated and not begun), with the
...
M3_2_G1_STAGE_STATUS: AUTHORIZED AND NOT BEGUN — accepted Decision 043, 2026-08-06, outcome M3_2_G1_NAVIGATION_AND_WORKFLOW_REPAIR_AUTHORIZED; no G1 byte has been written and none may be written before the separate owner-issued G1 implementation packet; envelope is the seven-path ceiling Docs/decision_index.md, Docs/change_impact_map.md, Docs/architecture_map.md, Milestones/STATUS.md, scripts/context_snapshot.sh, Makefile, and the new Docs/m3/review_execution_conventions.md, a ceiling and not a requirement to edit every path; at most one implementation commit with the exact subject Repair M3.2 navigation and review workflow; no tag; no T2.5 work in that commit; a separate fresh independent-review authorization follows, the G1 reviewer must not be the implementation session, and G1 acceptance and T2.5 implementation authorization remain separate owner judgments
...
IMPLEMENTATION_AUTHORIZATION: NO M3.2 CONTRACT T-SERIES IMPLEMENTATION STAGE IS CURRENTLY AUTHORIZED; THE BOUNDED NON-PRODUCTION STAGE M3.2 G1 (NAVIGATION AND WORKFLOW REPAIR) IS AUTHORIZED BY ACCEPTED DECISION 043 BUT HAS NOT BEGUN AND MAY NOT BEGIN BEFORE THE SEPARATE OWNER-ISSUED G1 IMPLEMENTATION PACKET, AND IT AUTHORIZES NO PRODUCTION SOURCE OR TEST BEHAVIOUR CHANGE, NO MIGRATION, NO RECEIPT CHANGE, NO REASON-CODE CHANGE, NO NETWORK ENABLEMENT, AND NO TAG — STAGE T2.4 IS ACCEPTED, COMPLETE, AND PUBLISHED AND ITS GRANT IS EXHAUSTED (stage authorized by accepted Decision 040, 2026-08-06, outcome M3_2_T2_4_IMPLEMENTATION_AUTHORIZED; correction authority recorded by accepted Decision 041, 2026-08-06, outcome M3_2_T2_4_RECOVERY_STATE_PRIMITIVE_AUTHORITY_RECORDED; accepted and published by accepted Decision 042, 2026-08-06, outcome M3_2_T2_4_ACCEPTED_AND_PUBLISHED). Stage T2.4 (recovery, reconciliation, resume boundaries, and drift control) was implemented within the maximum envelope Decision 041 §4 amended from eight paths to exactly ten for the correction only — src/disclosure_drift/m3/acquisition.py, src/disclosure_drift/m3/recovery.py, src/disclosure_drift/m3/__init__.py, src/disclosure_drift/reasons.py (the one-code SOURCE_REQUIRED_OBJECT_UNAVAILABLE registry delta only), src/disclosure_drift/sec/observation_catalog.py (released from Decision 040 §12 solely for the two additive public primitives open_recovery_state and resolve_recovery_state), tests/unit/test_m3_acquisition.py, tests/unit/test_m3_recover.py (the one authorized new test file), tests/unit/test_m3_recovery.py, tests/unit/test_reasons.py (expressly added to the Decision 035 envelope for T2.4 only), and tests/unit/test_observation_catalog.py (editable only to prove the new primitive pair) — and the accepted candidate 625c03d6931e01acc99946ca3924f1cda4da6b76 (tree 816fd392df859106b9ba21b684f9b4a8061461fc) changes exactly eight of those ten, leaving m3/recovery.py and tests/unit/test_m3_recovery.py unedited as the maximum-not-requirement rule permits, with no other previously prohibited path released and no eleventh path. Stage T2.1 is complete, owner-accepted, and published (accepted Decision 036; commit 7b2ffe643a2e2e600f148592fc9f8ded5695a279) and its grant is exhausted; combined stage T2.2-T2.3 is accepted, complete, and published (accepted Decision 039; candidate 6b189df1651ec3674ec7f96a1f5d66f488c654a9) and its grant is exhausted; combined T2.5-T2.6 remains owner-gated, unauthorized, and not begun, and T2.4 acceptance does not itself authorize it — a separate explicit owner stage authorization is required. No network or CompanyFacts enablement, SEC connectivity testing, HTTP request, live acquisition, real operational catalog, operational m3 acquire execution, receipt emission, private reconciliation-report creation, evidence indexing, ceiling-801 use, M3.2B activity, real snapshot, real manifest, root approval, publication, migration, receipt-schema change, further reason code, or tag work is authorized
