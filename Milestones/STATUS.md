...
**`CHATGPT_OWNER_M3_2_T2_4_IMPLEMENTATION_AUTHORIZATION_AFTER_T2_2_T2_3_PUBLICATION`** — the
owner's separate T2.4 implementation-authorization act, which requires its own exact packet and its
own owner decision. **The marker becomes operative once the authorized push of the three-commit
sequence succeeds**; it is set in the same governance commit because exactly one push is authorized
and no later commit may follow publication. **T2.4 remains unauthorized and not begun until that
separate owner act issues.**
...
T2.5–T2.6 remain owner-gated, unauthorized, and not begun**, with the T2.5–T2.6 commit still the
implementation-freeze candidate for the independent T3 review; T3, T4, and each per-window T5
remain separate later owner acts.
...
passed; no stage tag exists. **Seven obligations remain open** for later authorized stages: owner
