...
IMPLEMENTATION_AUTHORIZATION: THE DECISION-083 R46 MULTI-REGISTRANT RELATIONAL CORRECTION AND MIGRATION 0014, CONTINUED UNDER THE BOUNDED DECISION-084 AUTHORITY (accepted Decision 083 section 10; accepted Decision 084 R65-R67). Authorized paths are Decision 082 section 10.14's set PLUS EXACTLY TWO added by Decision 084: src/disclosure_drift/m3/acquisition.py for the R65 FINAL_MIGRATION_VERSION constant ONLY, and src/disclosure_drift/m3/offline_execution.py for the R66 paired_accessions_from_rows caller ONLY. Broadening either, or any other path, without owner action is PROHIBITED and is a STOP. Migration 0014 is PROSPECTIVE and PRE-E0: it is never applied to and never mutates the accepted private M3.2 operational catalog, it is exercised only through test, disposable, and repository migration machinery, and non-empty state requiring destructive reinterpretation is a STOP. PROHIBITED: cohorts.py; pilot_policy.py; migrations 0001-0013; preregistration.md; every existing record in Docs/Decisions/; network request logic; SEC transport behaviour; the D081 private verification evidence; the accepted M3.2 evidence root; migration 0015; the verified-evidence schema; and any document review or adjudication code. NO OTHER IMPLEMENTATION IS AUTHORIZED: migration 0015, the verified-evidence schema, REVIEW A, REVIEW B, the DOCUMENT ADJUDICATION, M3.3-E0, M3.3-E1, M3.3-E2, and M3.4 all remain UNAUTHORIZED, network, SEC, and HTTP authority is NONE with REQUEST_CEILING 0, and SUCCESSFUL IMPLEMENTATION IS NOT ACCEPTANCE — R49 condition B is satisfied only after a fresh independent Claude Fable 5 maximum review AND Sol/GPT owner acceptance
ACTIVE_STAGE_CONTRACT: Milestones/contracts/m3_3.md
NEXT_AUTHORIZED_ACTION: COMPLETE THE BOUNDED DECISION-084 CONTINUATION OF THE DECISION-083 R46 IMPLEMENTATION — AND NOTHING ELSE — THEN RETURN TO SOL/GPT. Accepted Decision 084 (2026-08-15, outcome D083_OWNER_ACTION_CONTINUATION_AUTHORIZED) resolves the single narrow owner-action stop the Decision-083 implementation hit at final validation. THE D083 IMPLEMENTATION IS COMPLETE AND PROVED AND MUST BE PRESERVED, NOT REDONE: MR-M1 THROUGH MR-M14 ALL PASS (18/18), E1-E8 ALL PASS (83/83), SINGLE_REGISTRANT_UNEXPECTED_IDENTITY_DELTAS = 0, THE AFFECTED IDENTITY INVENTORY DID NOT EXCEED E1-E5, AND EVERY STATIC GATE PASSES. The remaining work is exactly three things. FIRST, R65: change FINAL_MIGRATION_VERSION in src/disclosure_drift/m3/acquisition.py from 13 to 14 — THAT CONSTANT AND NOTHING ELSE IN THAT FILE — because migration 0014 moved the repository's schema chain head and prepare_operational_catalog now refuses every catalog it creates; this records a SCHEMA FACT ONLY and does NOT reopen M3.2, authorize acquisition, authorize network access, authorize applying migration 0014 to the accepted private M3.2 operational catalog, authorize writing accepted M3.2 evidence, move m3.2-complete, or grant M3.3-E0 authority; migration 0014 REMAINS PROSPECTIVE AND PRE-E0 and the accepted private M3.2 operational catalog REMAINS UNTOUCHED WITH NO INVOCATION AGAINST IT AUTHORIZED. SECOND, R66: correct src/disclosure_drift/m3/offline_execution.py STRICTLY AT THE CALLER AROUND paired_accessions_from_rows so the already-implemented association-aware support-target-pair logic receives the complete substantive registrant association set — established multi-registrant accessions attribute a valid 2009/2010 support/base pair to the appropriate substantive entity WITH NO ARBITRARY SCALAR ANCHOR, established single-registrant behaviour stays BYTE-FOR-BYTE IDENTICAL, an UNESTABLISHED set FAILS CLOSED AT ZERO PAIR CREDIT, and minimum/maximum CIK, first-write CIK, submitter, row order, date proximity, name, ticker, and hash order are ALL PROHIBITED fabrication routes; accession-domain deduplication stays by canonical accession and the pair quota, frozen eligible forms, 2009/2010 rule, and research methodology are UNCHANGED. THIRD, R67 requires NO code change: candidate_identity.py is NOT widened, pure single-registrant snapshots keep E1-E5 BYTE-IDENTICAL, and the fresh independent acceptance review MUST VERIFY that the relational set is genuinely governed and bound — no association removable or alterable without changing the appropriate governed digest — STOPPING rather than modifying candidate_identity.py if that claim is false. THEN run targeted validation, run EXACTLY ONE make check-fast at WORKERS=7 DIST=worksteal requiring ZERO FAILURES, and create ONE implementation commit — parented on the Decision-084 governance commit and EXCLUDING the Decision-084 governance files — containing the complete D083 implementation, migration 0014, all D083 authorized tests, the R65 constant, and the R66 bounded caller correction. PROHIBITED THROUGHOUT: cohorts.py; pilot_policy.py; migrations 0001-0013; preregistration.md; historical Decision records; network request logic; SEC transport behaviour; D081 private evidence; the accepted M3.2 evidence root; migration 0015; the verified-evidence schema; and any document-review or adjudication implementation — acquisition.py authority is ONLY the R65 constant and offline_execution.py authority is ONLY the R66 support-pair caller, and broadening either is a STOP. EVEN AFTER A CLEAN IMPLEMENTATION COMMIT, M3.3-E0, M3.3-E1, M3.3-E2, AND M3.4 REMAIN UNAUTHORIZED and R49 condition B is satisfied ONLY after a FRESH INDEPENDENT CLAUDE FABLE 5 MAXIMUM REVIEW AND SOL/GPT OWNER ACCEPTANCE — the implementing session does NOT self-review for formal acceptance. MIGRATION 0015, REVIEW A, REVIEW B, AND THE DOCUMENT ADJUDICATION REMAIN UNAUTHORIZED. M3_3_REAL_AMENDMENT_PURPOSE_FEASIBILITY_GATE_OPEN and M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE_OPEN both remain ACTIVE and are never merged into one flag, and M3_3_REAL_ACCEPTANCE_ORDERING_ADEQUACY is PENDING FUTURE AUTHORIZED E0 VERIFICATION. NETWORK, SEC, AND HTTP AUTHORITY IS NONE WITH REQUEST_CEILING 0; MIGRATION_AUTHORIZED is 0014 ONLY; and m3.2-complete remains unmoved with NO tag created
