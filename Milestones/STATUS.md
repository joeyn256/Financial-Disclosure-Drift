...
CURRENT_STAGE: M3.3 — THE D137 EXTERNAL WORKING-ROOT AND CAPACITY-SAFETY IMPLEMENTATION IS COMPLETE AND PUBLISHED; IT IS NOT OWNER-ACCEPTED AND IT AUTHORIZES NO CANARY. Decision 137 ships all nine D136-R11 items under implementation authorization M3_3_D137_EXTERNAL_WORKING_ROOT_AND_CAPACITY_SAFETY_IMPLEMENTATION_AUTHORIZED (issued outside this repository, now spent), classification BOUNDED_SAFETY_IMPLEMENTATION_ONLY, completion token M3_3_D137_IMPLEMENTATION_COMPLETE_READY_FOR_OWNER_REVIEW, AND NO ACCEPTANCE TOKEN. D136 QUALIFIED A VOLUME; D137 MAKES IT SAFE TO USE ONE; NEITHER ADOPTS IT AND NEITHER STARTS A RUN. NO SECOND ROOT-SELECTION MECHANISM WAS CREATED (D137-R2): the existing operator-supplied --work-root is sufficient and is REUSED UNCHANGED together with its accepted two-layer enforcement, because what was missing was never a way to NAME an external root but a way to AUTHENTICATE one — SQLITE_TMPDIR returned ZERO repository-wide matches before this stage. The guards live in ONE new module, src/disclosure_drift/m3/external_working_root.py, and reach the run through ONE optional parameter; OMITTED, EVERY PATH BEHAVES EXACTLY AS ACCEPTED DECISION 116 LEFT IT. IDENTITY IS FAIL-CLOSED ON THE VOLUME UUID (D137-R1): require_qualified_volume() authenticates against 397A4D4A-9508-391E-814E-3B533C7BD049 case-insensitively and otherwise exactly; disk4 AND disk4s2 ARE ATTACH-TIME AND ARE NEVER IDENTITY, and a mount path is not either — /Volumes/SSK SSD is whatever volume happens to be mounted there; WRONG UUID, MISSING VOLUME, AND LOOKUP FAILURE ARE THREE CAUSES WITH ONE OUTCOME, REFUSAL, AND THERE IS NO FALLBACK TO INTERNAL STORAGE; identity is read from diskutil info -plist as a PROPERTY LIST, never the human-oriented table, and the mount point is DERIVED by an st_dev walk because diskutil will not answer for a subdirectory. NO TEST DEPENDS ON THE OPERATOR'S SSD — every guard resolves its provider from module globals AT CALL TIME. ARCHIVE ISOLATION (D137-R3) refuses a root that IS, lies INSIDE, or CONTAINS the D130 archive, decided on realpath-resolved case-folded COMPONENTS, so '..', a symlink, and a case variant cannot launder containment AND a lawful sibling sharing the name prefix is NOT FALSELY REFUSED. THE 185 GiB LAUNCH FLOOR (D137-R4) 198,642,237,440 BYTES is measured on the filesystem hosting the SELECTED root; >= admits at the floor; NOTHING IS DELETED, MOVED, OR CLEANED TO REACH IT, and an unmeasurable volume REFUSES. THE 50 GiB PRE-F2 FLOOR (D137-R5) 53,687,091,200 BYTES NOW LIVES IN CODE: PRE_F2_MINIMUM_FREE_BYTES MOVED FROM 30 * 1024**3 TO 50 * 1024**3 — THE CONSTANT MOVED AND THE MECHANISM DID NOT, with the strict <, the call site between F1's return and F2's call (D126-R6), the refusal shape, and the Decision 094 §6.4 ordering byte-unchanged, and THE GUARD IS RETAINED, NOT REMOVED. THE OLD 30 GiB BEHAVIOUR HAS NO REACHABLE PATH and this is PROVED, not asserted: a test pins free space at EXACTLY 32,212,254,720 and requires a refusal. Decision 127's own test file is UPDATED IN PLACE, NOT DELETED AND NOT SKIPPED. CONTINUOUS MONITORING (D137-R6) adds a watchdog capacity subcommand: THE 10 GiB EMERGENCY FLOOR DID NOT MOVE (D124-R5) and alerting begins at 20 GiB, both INCLUSIVE; A HARD STOP DURING F2 IS A ROLLBACK, NOT A TRUNCATION, stated in the message itself; the subcommand REPORTS AND NEVER ACTS — no signal, no deletion, no cleanup, D131 no-escalation untouched — and its thresholds are IMPORTED from the package so a monitor cannot drift from the gate it monitors. PHASE EVIDENCE (D137-R7) records PRE_LAUNCH, POST_F0, PRE_F1, POST_F1_PRE_F2 and POST_F2 into the result document with an invented label REFUSED; DURING_F2 IS DELIBERATELY ABSENT IN-PROCESS because F2 is one blocking call inside one transaction and claiming it would invent a measurement never taken; AN UNKNOWN MEASUREMENT STAYS None, NEVER 0; a run with no external requirement records nothing and renders no key, so the accepted D119 byte-level evidence-equivalence is UNCHANGED. SQLITE_TMPDIR (D137-R8) is required explicitly, absolute, existing, outside the archive, and on the SAME QUALIFIED VOLUME — VALIDATED, NEVER SET BY LIBRARY CODE. THE BOUNDED D130 CHECK (D137-R10) hashes the four D130 §6 compact proofs and STATs the 103,966,696,960-BYTE TAR WITHOUT EVER OPENING IT, proved by a TRIPWIRE OVER Path.read_bytes rather than by reading the source. VALIDATION: focused baseline 317 PASSED before any edit and 384 PASSED after (317 + 65 + 2, NO REGRESSION); 17 REVERSIBLE SOURCE MUTATIONS, EVERY ONE CAUGHT BY NAMED TESTS, tree verified byte-identical afterwards, NO SOURCE-MUTATING FRAMEWORK USED. ONE BOUNDED READ-ONLY LIVE PREFLIGHT RAN: UUID EXACT MATCH, archive present with 24 ENTRIES and NO DIFFERENCES in 0.171 s, free 310,498,951,168 BYTES / 289.1747 GiB against the 185 GiB floor with SURPLUS 111,856,713,728 BYTES / 104.1747 GiB, and the archive-child, volume-root, unset-SQLITE_TMPDIR and internal-SQLITE_TMPDIR controls ALL REFUSED, with NOTHING CREATED, WRITTEN, DELETED OR BENCHMARKED ON THE VOLUME AND THE TAR NEVER OPENED; free space is 393,216 BYTES — three 131,072-BYTE ExFAT allocation blocks — HIGHER than D136 §5 recorded, stated rather than rounded away, moving no floor and no verdict. NO OVERCLAIM (D137-R11): NO journaled-filesystem semantics, NO power-loss safety, NO surprise-removal safety, NO USB-bridge cache-flush correctness — D136's PROCESS-CRASH-RECOVERY-ONLY boundary is preserved exactly. THE LIVE SQLITE_TMPDIR DIRECTORY WAS NOT CREATED because that is a write, so the guard's ACCEPTING path is proved SYNTHETICALLY and only its REFUSING paths ran live. NO MIGRATION: HEAD REMAINS 0015 AND 0016 IS ABSENT, UNAPPLIED AND NOT AUTHORIZED. ALL THREE ACTIVATION CONSTANTS REMAIN None. NETWORK REMAINS DISABLED at both tracked switches with request ceiling 0. THE D131 RUNTIME CONFIGURATION, PARSER AND SHARD-DISPATCH SEMANTICS ARE UNTOUCHED AND THE D134 mmap AND RELAXED-CHECKPOINT CANDIDATES REMAIN REJECTED. census_orchestrator.py::_parse_bulk REMAINS AN EXPLICIT PRE-NETWORK BLOCKER, DELIBERATELY UNREPAIRED. D129-R2, D129-R8 AND D129-R12 ARE NOT DISCHARGED. A COMPLETE-SOURCE CANARY IS NOT AUTHORIZED. E0 IS NOT AUTHORIZED.
ACTIVE_BLOCKER: THE CORRECTED COMPLETE-SOURCE CANARY REMAINS UNAUTHORIZED. D137's implementation is COMPLETE but NOT INDEPENDENTLY REVIEWED AND NOT OWNER ACCEPTED, and the D136-R8 narrow one-canary exception takes effect ONLY AFTER D137 IMPLEMENTATION AND ACCEPTANCE — implementation alone moves nothing. A passing preflight prints canary_authorized: false, which is a fact and not a formality. FOUR RESIDUAL LIMITATIONS ARE CARRIED RATHER THAN CLOSED: (1) DURING_F2 CANNOT BE SAMPLED IN-PROCESS and depends on the operator running the watchdog capacity subcommand alongside the run, which nothing forces; (2) THE EXTERNAL REQUIREMENT IS OPT-IN — omitting --require-volume-uuid restores the pre-D137 behaviour exactly, which is what keeps every existing internal-volume canary path working, but means the guards protect a run that ASKS to be protected; (3) THE SQLITE_TMPDIR GUARD VALIDATES PLACEMENT, NOT PEAK — peak spill is recorded at phase boundaries when a run takes them and is not bounded in advance, and D136-R5 left it unmeasured; (4) NO LIVE END-TO-END REHEARSAL WAS PERFORMED because that would require creating a world on the volume, so the composed preflight's ACCEPTING path is proved synthetically and by its live REFUSALS, not by a live pass. SEPARATELY AND INDEPENDENTLY, census_orchestrator.py::_parse_bulk REMAINS AN OPEN PRE-NETWORK BLOCKER whose repair MUST NOT be performed as a side effect of unrelated work.
IMPLEMENTATION_AUTHORIZATION: SPENT — the D137 packet's authorization M3_3_D137_EXTERNAL_WORKING_ROOT_AND_CAPACITY_SAFETY_IMPLEMENTATION_AUTHORIZED was issued outside this repository and is now consumed by this implementation. NO FURTHER IMPLEMENTATION AUTHORITY ARISES FROM THE DECISION 137 PUBLICATION. It does NOT adopt the external volume, does NOT create a canary world, a run identity, an execution namespace, a launch receipt, or a terminal state, does NOT authorize a corrected complete-source canary or any canary, does NOT authorize E0, does NOT create or authorize migration 0016, does NOT authorize network, SEC or HTTP at request ceiling 0, does NOT repair census_orchestrator.py::_parse_bulk, does NOT change D131 parser or runtime semantics, does NOT adopt any D134 candidate, and certifies NO semantic count. IT CLAIMS NO OWNER ACCEPTANCE. SELECTING A PATH, QUALIFYING A VOLUME, AND ADOPTING IT ARE THREE DISTINCT STEPS — D135 SELECTED, D136 QUALIFIED, AND D137 BUILDS WHAT ADOPTION WILL NEED WITHOUT TAKING IT.
NEXT_AUTHORIZED_ACTION: INDEPENDENT REVIEW OF THE DECISION 137 IMPLEMENTATION — AND NOTHING ELSE. It is not started and must not be started from this publication. A session that creates a canary world, a run identity, an execution namespace or a launch receipt, that writes to the SSK SSD, that sets any activation constant, that adds migration 0016, or that enables network on the strength of D137 alone is acting OUTSIDE D137. THE SSD MUST NOT BE WRITTEN TO under this publication, the D130 archive MUST NOT BE MODIFIED, MOVED, DELETED OR HASHED, and the large D130 tar MUST NOT BE OPENED. After review, an OWNER DECISION ON WHETHER TO ACCEPT D137 — and, separately and only then, whether to authorize the corrected canary.
