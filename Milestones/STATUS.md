...
IMPLEMENTATION_AUTHORIZATION: THE BOUNDED CORRECTION OF THE SIX ACCEPTED D087 INDEPENDENT-REVIEW FINDINGS — M-1, MIN-1, MIN-2, MIN-3, OBS-2, AND OBS-3 — AND NOTHING ELSE (accepted Decision 088 sections 2-7 and section 10). THE D087 INDEPENDENT REVIEW RETURNED FAIL at BLOCKER 0 / MAJOR 1 / MINOR 3 / OBSERVATION 3 against frozen target 8c13fc79aee649df4956643f0b24504c8cdfd2c7 (tree 80dc6c051641551e6b53ffd02a41f94db4d8a6d6), token M3_3_DECISION_087_VERIFIED_EVIDENCE_SCHEMA_INDEPENDENT_REVIEW_FAIL, and THAT VERDICT IS FROZEN AND IMMUTABLE — it was reached and reported BEFORE any correction authority existed and is neither revised nor softened. THE ACCEPTED ARCHITECTURE IS NOT REOPENED AND NO REDESIGN IS AUTHORIZED: the review independently confirmed applicability enforcement at both the schema and module layers, no verified_amends_original state, correct reviewer-epoch independence in both directions, private-root nonleakage with no locator column, seven new hash domains through the existing release/hashing.py with no frozen tuple widened, and the accepted R68 chain movement reproduced to the byte with candidate_tables_sha256 and selection_result_sha256 byte-identical. M-1 IS ACCEPTANCE-GATING: SQLite resolves an INSERT OR REPLACE conflict by deleting the conflicting row and inserting the new one, and that implicit delete fires no BEFORE DELETE trigger unless PRAGMA recursive_triggers is on, which this project NEVER SETS — so a frozen adjudicated result, a review record's role and epoch, span provenance, and artifact metadata were each REWRITTEN while every existing protection stayed silent. It is closed by the accepted migration-0013 BEFORE INSERT REPLACEMENT-GUARD pattern over EVERY unique route of ALL FOUR relations, refusing INSERT OR REPLACE, an ordinary duplicate INSERT, and a silent INSERT OR IGNORE alike, with the existing BEFORE UPDATE and BEFORE DELETE protections KEPT rather than replaced. MIN-1: every document_review_records and document_adjudicated_evidence row must bind an artifact whose REGISTERED accession equals its own, with NO new accession identity invented. MIN-2: agreement_state 'agreed' requires BOTH contributing reviews to be NON-ABSTAINING and to carry the asserted adjudicated value per evidence kind (Decision 082 section 12.6), WITHOUT turning abstention into a negative assertion or fabricating evidence from absence, and the verified-implies-agreed-or-resolved CHECK receives a DEDICATED NEGATIVE TEST because its removal previously survived every existing test. MIN-3: the verified-candidate guard must ALSO fire when accession_plain changes while the level stays verified — by naming the additional column, NOT by candidate-identity redesign, and verified applicability is NOT widened. OBS-2 is a COMMENT CORRECTION ONLY. OBS-3 strengthens span_location to strict bytes:<decimal>-<decimal> with NO fuzzy source-location semantics. OBS-1 REMAINS OPEN: it is ACCEPTED AS NON-GATING AND DEFERRED and MUST NOT be reported as fixed or closed. Authorized paths are exactly the accepted Decision 088 section 10 list: migration 0015 CORRECTED IN PLACE, tests/unit/test_m3_3_verified_document_evidence.py, the tests/unit/test_m23_pilot_manifest_store.py policy-chain re-baseline only if the final bytes move it, Docs/sec_data_dictionary.md, Docs/architecture_map.md, Docs/change_impact_map.md, Docs/decision_index.md, Docs/Decisions/decision_registry.md, and Milestones/STATUS.md. MIGRATION 0016 IS NOT AUTHORIZED. Prohibited: cohorts.py, pilot_policy.py, candidate_identity.py, candidate_snapshot.py, offline_execution.py beyond the accepted R66 caller, acquisition.py beyond the current migration-head truth, release/hashing.py, migrations 0001-0014, preregistration.md, Decisions 001-087, every prior independent-review artifact, the accepted D081 evidence, the accepted M3.2 evidence root, every network/acquisition/transport module, candidate-selection methodology, document-classification logic, real review execution, and E0. Required identity nonchange: only the accepted R68 policy-chain path may move, with candidate_tables_sha256, selection_result_sha256, source_observation_set_sha256, quota_definitions_sha256, selected_entities_sha256, selected_accessions_sha256, reserves_sha256, and quota_report_sha256 ALL BYTE-IDENTICAL and anything else moving a STOP. VE-M1 through VE-M14 are RE-RUN and VE-R1 through VE-R10 are added, DEMONSTRATED rather than named. ROLE BOUNDARY: the same Fable epoch that produced the failed review acts ONLY as the owner-authorized correction executor and is NOT ELIGIBLE to accept its own corrected target — the acceptance rereview must be a FRESH /clear epoch. CORRECTION IS NOT ACCEPTANCE: D087_VERIFIED_EVIDENCE_SCHEMA remains NOT YET OWNER ACCEPTED, and REVIEW A, REVIEW B, THE DOCUMENT ADJUDICATION, M3.3-E0, M3.3-E1, M3.3-E2, and M3.4 all remain UNAUTHORIZED with network/SEC/HTTP NONE at REQUEST_CEILING 0
ACTIVE_STAGE_CONTRACT: Milestones/contracts/m3_3.md
NEXT_AUTHORIZED_ACTION: RETURN TO SOL/GPT FOR A FRESH INDEPENDENT ACCEPTANCE REREVIEW OF THE CORRECTED VERIFIED-EVIDENCE SCHEMA — AND NOTHING ELSE. The accepted Decision 088 correction stage is COMPLETE: migration 0015 is corrected IN PLACE and all six accepted findings are closed with demonstrated effectiveness. M-1 CLOSED — four BEFORE INSERT replacement guards on the accepted migration-0013 pattern cover EVERY unique route of ALL FOUR relations (document_artifacts PK and UNIQUE(accession_plain, source_class); document_review_records PK and UNIQUE(accession_plain, reviewer_role); document_review_spans PK; document_adjudicated_evidence PK) and refuse INSERT OR REPLACE, an ordinary duplicate INSERT, and a silent INSERT OR IGNORE alike, with the eight BEFORE UPDATE/DELETE triggers KEPT; the exact probes that demonstrated the defect now report BLOCKED and the frozen values are unchanged. MIN-1 CLOSED — document_review_records_bind_their_own_accession and document_adjudicated_evidence_binds_its_own_accession require the bound artifact to be REGISTERED to the row's own accession, each proven to do independent work (with the review-side guard removed the adjudication-side guard still refuses). MIN-2 CLOSED — document_adjudicated_evidence_agreed_requires_agreeing_passes refuses an 'agreed' outcome unless BOTH passes are non-abstaining AND assert the adjudicated value per kind, with abstention NOT turned into a negative assertion and the lawful abstained/conflicting/resolved routes intact, plus a DEDICATED NEGATIVE TEST for the verified-implies-agreed-or-resolved CHECK. MIN-3 CLOSED — the verified-candidate UPDATE trigger now watches amendment_purpose_evidence_level AND accession_plain. OBS-2 CLOSED (comment only). OBS-3 CLOSED — span_location is strict bytes:<decimal>-<decimal>. OBS-1 REMAINS OPEN AND DEFERRED and is pinned as open by its own test. PROVED: 122 verified-evidence tests pass (82 before); VE-M1 through VE-M14 re-run and still EFFECTIVE; VE-R1 through VE-R10 added and ALL TEN correction guards independently DEMONSTRATED LOAD-BEARING by removal with ZERO vacuous; a 45-case post-correction door battery at 0 unexpected; and the lawful lifecycle positive controls (agreed route, resolved route with a third epoch, one epoch reviewing many accessions, verified candidate consumption) all still ADMITTED. IDENTITY: the ONLY movement is the accepted R68 migration-chain policy binding driven solely by the corrected 0015 checksum (c53288947720f397cbb5e9661767bd37a67dbde76170bb7089df28d364d45593 -> d7f22999cb3e6736c765de72a1031c170f2cb5547ccaccf7469a2d3be018835f) — selector_policy_sha256, root_manifest_sha256, and manifest_id — while the canonical-JSON length is UNCHANGED at 275721 and candidate_tables_sha256, selection_result_sha256, source_observation_set_sha256, quota_definitions_sha256, selected_entities_sha256, selected_accessions_sha256, reserves_sha256, and quota_report_sha256 are ALL BYTE-IDENTICAL; no frozen identity tuple is widened and no candidate column is added. ONE routine make check-fast returned exit 0. THE CORRECTING SESSION IS NOT ELIGIBLE TO ACCEPT ITS OWN TARGET: it previously served as the D087 independent reviewer, that FAIL verdict was frozen before any correction authority existed, and Decision 088 changed its role to correction executor only. THE ACCEPTANCE REREVIEW MUST BE A FRESH /clear EPOCH that inherits none of the correction session's conclusions. UNTIL THAT LANDS: D087_VERIFIED_EVIDENCE_SCHEMA is NOT YET OWNER ACCEPTED; DOCUMENT REVIEW A, DOCUMENT REVIEW B, AND THE DOCUMENT ADJUDICATION REMAIN UNAUTHORIZED; M3.3-E0, M3.3-E1, M3.3-E2, AND M3.4 REMAIN UNAUTHORIZED; the 108 real D081 review outcomes are NOT inserted and the D081 private evidence was NOT accessed; M3_3_REAL_AMENDMENT_PURPOSE_FEASIBILITY_GATE_OPEN and M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE_OPEN both remain ACTIVE and are never merged into one flag; M3_3_REAL_ACCEPTANCE_ORDERING_ADEQUACY is PENDING FUTURE AUTHORIZED E0 VERIFICATION; NETWORK, SEC, AND HTTP AUTHORITY IS NONE WITH REQUEST_CEILING 0; MIGRATION_AUTHORIZED is NONE (the 0015 correction is complete and committed) with 0016 ABSENT; and m3.2-complete remains unmoved with NO tag created
...
M3_3_VERIFIED_EVIDENCE_SCHEMA_IMPLEMENTATION_STATUS: IMPLEMENTED AND INDEPENDENTLY REVIEWED — REVIEW RETURNED FAIL; UNDER BOUNDED OWNER-AUTHORIZED CORRECTION (implementation authorized by accepted Decision 087 sections 4 and 13; the review adjudication and correction authority at accepted Decision 088). The fresh independent Claude Fable 5 maximum review of the frozen implementation target 8c13fc79 returned FAIL at BLOCKER 0 / MAJOR 1 / MINOR 3 / OBSERVATION 3 (token M3_3_DECISION_087_VERIFIED_EVIDENCE_SCHEMA_INDEPENDENT_REVIEW_FAIL), and THAT VERDICT IS FROZEN AND IMMUTABLE. The migration chain is 0001-0015 contiguous with 0001-0014 BYTE-UNCHANGED; the four relations still ship EMPTY and are exercised only by synthetic disposable fixtures under pytest tmp_path; NO real D081 evidence is inserted and the D081 private evidence artifacts were NOT accessed. CONFIRMED CORRECT BY THE REVIEW AND NOT REOPENED: no persisted EV_ROOT/private/local-user/scratch path and no locator column at all; opaque 64-hex review epochs with UNIQUE (accession_plain, reviewer_role) plus the one-role-per-epoch trigger forcing three distinct epochs per accession while one epoch may still review all 108 artifacts; evidence_level 'verified' admitted ONLY for amendment purpose and amendment linkage / explicit-original, enforced at the evidence_kind CHECK AND by document_evidence.require_verified_evidence_applicable; NO verified_amends_original value anywhere; a verified candidate level requiring a frozen adjudicated row; and seven NEW hash domains through the existing release/hashing.py with NO frozen tuple widened. UNDER CORRECTION per accepted Decision 088: M-1 the INSERT OR REPLACE implicit-delete rewrite door on all four relations (ACCEPTANCE-GATING, closed by the accepted migration-0013 BEFORE INSERT replacement-guard pattern with the existing UPDATE/DELETE protections KEPT); MIN-1 cross-accession artifact binding; MIN-2 agreed-state consistency plus a dedicated negative test for the verified-implies-agreed-or-resolved CHECK; MIN-3 the verified-candidate accession re-point door; OBS-2 the section 1 comment; and OBS-3 strict bytes:<decimal>-<decimal> span locations. OBS-1 (non-canonical contributor-JSON encodings) is DEFERRED AND REMAINS OPEN and must NOT be reported as fixed. MIGRATION 0016 IS NOT AUTHORIZED — 0015 is corrected IN PLACE. NEXT: fresh /clear independent acceptance rereview by a session that did NOT perform the correction, then Sol/GPT owner acceptance. D087_VERIFIED_EVIDENCE_SCHEMA IS NOT YET OWNER ACCEPTED, and REVIEW A, REVIEW B, THE DOCUMENT ADJUDICATION, E0, E1, E2, AND M3.4 ALL REMAIN UNAUTHORIZED
