...
CURRENT_STAGE: M3.3 — DECISION 149 IS THE FINAL PRE-CANARY LIMITATION CLEANUP, PERFORMED UNDER OWNER ADJUDICATION AFTER DECISION 148 WAS ACCEPTED AS A PASS WITH NONBLOCKING LIMITATIONS (M3_3_D148_FINAL_INDEPENDENT_REREVIEW_OWNER_ACCEPTED). AUTHORIZATION M3_3_D149_FINAL_PRE_CANARY_LIMITATION_CLEANUP_AUTHORIZED, SPENT BY THIS PUBLICATION. ENTRY 1e03b44a2d97da0e428afcd179c13bd9aacf58e7 / TREE 153e4ef1eedac2bbb7a0b9f87d5e5111777f7b2d, CI 32726543966 SUCCESS ON BOTH MANDATORY JOBS — every material predicate matched and nothing was repaired to make it match. IT IS SMALL ON PURPOSE: two production edits, one new test module, and the runbook section the second edit made untrue — AND NO ACCEPTED TEST WAS CHANGED, DELETED OR SKIPPED, which is a MEASURED fact rather than an intention, because the blast radius of the only behavioural change was measured before it was designed. D149-R1 CLOSES D148-MINOR-1: the _git docstring claimed that resolving the executable to an absolute path made PATH order irrelevant, and shutil.which SELECTS BY PATH ORDER — it now states the property that is true, that the resolved executable is what runs with no second exec-time lookup, and names HOST TRUST as the boundary rather than implying the module removes it. NO PRODUCTION BEHAVIOUR CHANGED, re-derived from the module's own AST, and a documentation scan over every file in src/ plus the operator runbook fails if the false claim ever reappears in any wording. D149-R2 CLOSES D148-L3: the four deleted and renamed tracked-path states are covered directly against REAL git — deleted in the worktree; staged for deletion with HEAD^{tree} asserted NOT to have moved; renamed with git mv, reporting the NEW path because that is the file now on disk; and an unstaged worktree rename refused TWICE OVER — each driven through require_clean_running_repository to prove it FAILS CLOSED rather than merely reports, with a clean-repository positive control. Behaviour was already correct; only the evidence was missing, and repository identity is NOT redesigned. D149-R3 CLOSES D148-L2: --mode run is now REFUSED BY PRODUCTION on any working root the external envelope governs — one refusal, at the operator surface, taken immediately after the envelope is established and BEFORE the run is entered, KEYED ON THE ENVELOPE because D138-R1 already makes the resolved root the single thing that decides whether a run is governed and external. ON AN INTERNAL ROOT --mode run IS UNTOUCHED and remains the accepted Decision 116 path — the bounded library and development exercise the authorization expressly permits keeping, which cannot be mistaken for the governed route because the governed route is external by construction. SIX DIRECT PROOFS: the fully qualified external launch refuses and creates no world; phase-f0, phase-f1 and phase-f2 each reach ADMISSION on that same qualified configuration; cli.py reaches the canary through exactly one function and the whole-run entry point has exactly one caller, with an AST tripwire pinning the guard ABOVE the call; and no authority is created. THE D149-R3 STOP CONDITION WAS NOT REACHED — no architecture redesign was required. D149-R4 KEEPS D148-L1 ACCEPTED as D149-R4_ACCEPTED_LOCAL_CHECKOUT_WRITE_THREAT_MODEL_LIMITATION, verified only not to be broadened or made easier to reach. D149-R5 re-confirms census_orchestrator.py::_parse_bulk as DEFERRED_PRE_NETWORK_BLOCKER / NONBLOCKING_FOR_OFFLINE_CANARY, unrepaired. FALSIFICATION 7 MUTATIONS, 7 KILLED — and M5, which drops the mode == "run" condition so the guard catches every mode, matters as much as M4: it is what proves the refusal is NARROW rather than merely present. Every file restored byte-identical by SHA-256. THREE PHASES, TWO QUALIFIED BOUNDARIES, PHASE_BOUNDARY_RAM_RECLAMATION = IMPLEMENTED, REPOSITORY IDENTITY, DOCK-ONLY NARROWING AT FOUR SEAMS AND THE CHECKPOINT CONTRACT ARE ALL UNCHANGED. GOVERNED_PAUSE_RESUME and SAFE_TO_EJECT remain NOT_IMPLEMENTED. CANARY_AUTHORIZED = NO.
ACTIVE_BLOCKER: THE COMPLETE-SOURCE CANARY REMAINS UNAUTHORIZED. DECISION 149 CORRECTS THREE LIMITATIONS; IT ACCEPTS NOTHING AND AUTHORIZES NOTHING. THE CORRECTION ITSELF NOW NEEDS AN INDEPENDENT REVIEW, AND THE SESSION THAT WROTE IT IS FORBIDDEN TO PERFORM THAT REVIEW — the same rule that produced Decisions 146 and 148. Decision 146's FAIL verdict still stands against the tree it reviewed, Decision 143's against its own, and neither record is rewritten. Decisions 137 and 138 remain IMPLEMENTED — PENDING INDEPENDENT REVIEW AND OWNER ACCEPTANCE, and Decisions 140 and 141 remain accepted FOR CONTINUATION ONLY. THE FIRST REAL CANARY NEEDS ITS OWN OWNER INSTRUMENT, WHICH DOES NOT EXIST. census_orchestrator.py::_parse_bulk REMAINS AN OPEN PRE-NETWORK BLOCKER, re-traced and deliberately unrepaired. ONE NONACTIONABLE LIMITATION IS CARRIED BY OWNER ADJUDICATION: D148-L1 / D149-R4, the UNCHECKED_HASH byte-code case beneath an ignored __pycache__, which needs write capability inside the checkout and is outside the accepted canary threat model. THE ACCEPTED PHYSICAL LIMITATIONS ARE UNCHANGED — ExFAT has no metadata journal, surprise removal is not covered, the lid must stay open, corrected-scale RSS is measurable only during the real run, capacity observation is bounded rather than instantaneous, and hardware or dock failure remains physically possible. TWO OPERATOR-FACING CONSEQUENCES ARE NOT OPTIONAL: the checkout must be at the intended revision, with no uncommitted tracked change and no untracked non-ignored file, BEFORE phase-f0 and for the whole sequence; and --mode run IS NOW REFUSED on any externally governed root, so the governed sequence is phase-f0, phase-f1, phase-f2 and nothing else. A passing preflight still prints canary_authorized: false.
IMPLEMENTATION_AUTHORIZATION: SPENT — M3_3_D149_FINAL_PRE_CANARY_LIMITATION_CLEANUP_AUTHORIZED, together with the accepted predecessor M3_3_D148_FINAL_INDEPENDENT_REREVIEW_OWNER_ACCEPTED, was issued outside this repository and is now consumed by the publication of Decision 149. IT AUTHORIZED EXACTLY ONE BOUNDED CLEANUP STAGE: the correction of D148-MINOR-1, the direct test closure of D148-L3, the smallest mechanically sound hardening closing D148-L2, the explicit dispositions of D148-L1 and _parse_bulk, the tests and bounded falsification proving them, and the publication of this record — AND NOTHING ELSE. It did NOT authorize starting the canary, any phase mode against a real canary world, E0, the pre-E0 catalog transition, stale-writer-lease recovery, SEC acquisition, either network gate, minting any execution authority, creating migration 0016, opening D130 archive content, a physical storage disconnection, a re-run of the D141 qualification, or pause/resume implementation. NO NEW AUTHORIZATION EXISTS.
NEXT_AUTHORIZED_ACTION: STOP AND RETURN DECISION 149, WITH DECISIONS 145 THROUGH 148, TO GPT-5.6 SOL — AND NOTHING ELSE. DO NOT START THE CANARY. DO NOT PERFORM THE INDEPENDENT REVIEW IN THE SESSION THAT WROTE THIS CLEANUP. DO NOT BEGIN ANOTHER DECISION. DO NOT BEGIN E0. DO NOT ENABLE NETWORK. DO NOT PERFORM SEC ACQUISITION. THE NEXT INTENDED ACTION IS ONE FRESH CLAUDE FABLE 5 SESSION AT MAXIMUM EFFORT, FRESH /clear, PERFORMING THE FINAL INDEPENDENT PRE-CANARY REVIEW OF THE DECISION 149 IMPLEMENTATION TREE. THE FORWARD REQUIREMENT RECORDED BY DECISION 146 AND RECONFIRMED BY DECISION 148 IS NOW MECHANICALLY ENFORCED BY DECISION 149 D149-R3: the future real-canary authorization MUST authorize only --mode phase-f0, phase-f1 and phase-f2, each in its own process with a clean exit between them, and --mode run against an externally governed root is refused by production rather than only forbidden by governance.
