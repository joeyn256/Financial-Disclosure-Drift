...
CURRENT_STAGE: M3.3 PRE-E0 — GOVERNANCE IS RECONCILED THROUGH D124, AND THE CAPACITY MODEL IS NOW CLOSED RATHER THAN CONFLICTED. D124 ACCEPTED (M3_3_D124_CAPACITY_RECONCILIATION_OWNER_ACCEPTED_WITH_CORRECTION), SCOPE THE COMPLETE FIRST SOURCE ONLY AND EXPRESSLY NOT 76-SOURCE E0 CAPACITY EVIDENCE. BOTH DECISION 123 SECTION 9 CONFLICTS ARE CLOSED. CONFLICT A — MEMBER COUNT: the older sec_bulk_submissions-9ca4642200dbcc45.zip holds 985,480 raw entries and 985,479 governed JSON members; the current controlling sec_bulk_submissions-c85744be921b0dc5.zip holds 985,835 raw and 985,834 governed; each carries EXACTLY ONE non-JSON placeholder.txt, so the 355-member difference is TWO DIFFERENT SEC SNAPSHOT OBJECTS — NOT a filter, parser, or code discrepancy. D124-R1: 985,834 IS FINAL controlling and 985,479 IS SUPERSEDED-BY-OBJECT, NOT ERRONEOUS. CONFLICT B — STORAGE CONTRACT: the Decision 112 section 6 about 132.5 GB figure is e0-compact-evidence/1 with the RESOLUTION LAYER STILL UNCOMPACTED, while Decision 113 e0-compact-evidence/2 ADDITIONALLY COMPACTS RESOLUTION AND DERIVED EVIDENCE — SUCCESSIVE EVIDENCE CONTRACTS, NOT CONTRADICTORY PREDICTIONS OF ONE IMPLEMENTATION. D124-R2: retire the 132.5 GB figure FROM PLANNING ONLY, with /2 calibrated by D118 and the later real D120/D122/D123 evidence controlling, and Decision 112 NOT retired as historical evidence. ONE ERROR IS PUBLISHED RATHER THAN INHERITED (D124-R4): over the D122 section 5 8,258,521 accessions, 638,071/8,258,521 = 0.077262 is the EXPLICIT-ACCESSION FRACTION at ABOUT 7.73%, while 5,104,568/8,258,521 = 0.618097 is FIELD-RESOLUTION ROWS PER ACCESSION because one explicit accession can emit MULTIPLE field rows — so the reviewer's claim that 61.8% OF ACCESSIONS WERE EXPLICIT IS NOT PUBLISHED, NO CLAIM THAT THE TAIL BECOMES MORE IMPLICIT IS PUBLISHED, AND LINEAR F1 BYTE EXTRAPOLATION IS NOT CHARACTERIZED AS CONSERVATIVE-HIGH ON THAT BASIS; the linear F1 central projection survives because it rests on MEASURED BYTES PER ACCESSION. ACCEPTED CAPACITY MODEL, PLANNING ESTIMATES AND NOT COMPLETE-SOURCE OBSERVATIONS: materialization durable about 54.62 GB; compact sidecar about 0.27 GB; F1 durable growth about 9.40 GB at about 1.33 h; F2 durable growth about 6.59 GB with pre-checkpoint WAL about 9.52 GB at about 2.04 h and RSS about 0.68 GiB; central peak about 75.27 GiB and conservative modeled peak about 89.27 GiB. GATES (D124-R5): HARD STARTING INTERNAL FREE-SPACE GATE >= 105 GiB = 112,742,891,520 BYTES, superseding the reviewer's recommended 100 GiB; continuous hard-stop floor 10 GiB; >= 30 GiB REQUIRED IMMEDIATELY BEFORE OPENING F2 AND F2 MUST NOT BEGIN BELOW IT; NO VACUUM; SQLite temporary storage explicitly placed and accounted on the internal run volume. THE D121 SECTION 7 >= 85 GiB FLOOR IS SUPERSEDED, NOT MERELY SUSPENDED. EXTERNAL SSD (D124-R6): the exFAT SSK SSD is approved IN PRINCIPLE AS ARCHIVAL/COLD-PRESERVATION STORAGE ONLY; ACTIVE SQLITE USE IS NOT APPROVED; reformatting is neither approved nor required; the about 1050 MB/s figure is USER-REPORTED AND NOT INDEPENDENTLY BENCHMARKED; and the internal APFS Apple SSD REMAINS THE INTENDED ACTIVE WORKING VOLUME. STORAGE PRESERVATION AND RECLAMATION IS THE NEXT STAGE. MIGRATION HEAD REMAINS 0015 and 0016 is absent. E0-v3 REMAINS UNAUTHORIZED. NO COMPLETE-SOURCE RUN IS AUTHORIZED. NO ARCHIVAL, COPY, OR DELETION IS AUTHORIZED BY D124 — section 11 eligibility is NOT authorization and no D125 instrument exists yet.
ACTIVE_BLOCKER: INTERNAL FREE SPACE — PLAN A FALLS ABOUT 6.54 GiB SHORT OF THE OWNER GATE, AND NO REAL EXECUTION AUTHORIZATION OF ANY KIND EXISTS IN THIS REPOSITORY. The capacity/population model is NO LONGER the blocker: D124 closed both Decision 123 section 9 conflicts. What blocks now is arithmetic. PLAN A: clone-aware internal reclaim estimate about 61.77 GiB, current D124 internal free about 36.70 GiB, projected after archival plus authorized internal cleanup about 98.46 GiB — WHICH DOES NOT SATISFY THE 105 GiB GATE, leaving a gap of about 6.54 GiB. Summing the two published rounded inputs gives about 98.47 GiB, a rounding artifact of about 0.01 GiB that changes no conclusion. OWNER PREFERENCE: free or move ORDINARY NON-PROJECT DATA later rather than alter append-only project raw evidence to close a gap this small; NO NON-PROJECT DELETION IS AUTHORIZED OR PERFORMED. BEFORE ANY COMPLETE-SOURCE AUTHORIZATION (D124-R10): archival must complete; authorized internal copies must be reclaimed; internal free space must be MEASURED at >= 105 GiB — MEASURED, NOT PROJECTED; and all other future-run preflight predicates must pass. Nothing in this repository is an execution authorization: not this file, not a passing gate, not a commit, not a push, and not the D124 record published here — a record that resolves a capacity question is NOT a grant to spend the capacity. D118 R23 still binds. The complete-source canary, the full-population F2 rerun, the D117 retry, the three-source canary, and the real replay proof remain NOT RUN and NOT AUTHORIZED. All three execution authorities are None, migration 0016 is unapplied and the operational catalog stays at head 0015, no E0-v3 namespace exists, both tracked network switches stay false at request ceiling 0, and the preserved D117, D120 and D122 worlds plus the D123 characterization residue ALL STAY UNDELETED — D124 makes them ELIGIBLE UNDER A LATER INSTRUMENT and ELIGIBILITY IS NOT AUTHORIZATION (D124-R7).
IMPLEMENTATION_AUTHORIZATION: EXERCISED FOR GOVERNANCE PUBLICATION ONLY — THE OWNER'S 2026-08-20 DECISION 124 CAPACITY RECONCILIATION OWNER-ACCEPTANCE / GOVERNANCE PUBLICATION INSTRUCTION, TOKEN M3_3_D124_CAPACITY_RECONCILIATION_OWNER_ACCEPTED_WITH_CORRECTION, AUTHORIZED THIS DOCUMENTATION-ONLY PUBLICATION AND NOTHING ELSE, by expressly directing the creation or update of Decision 124, the decision registry, the decision index, and STATUS. The touched paths are Docs/Decisions/decision_124_m3_3_capacity_reconciliation.md (new), one Docs/Decisions/decision_registry.md index row, nine Docs/decision_index.md navigation rows, and this file. NO SOURCE-CODE CHANGE, no test change, no schema or migration change, and no Docs/change_impact_map.md change were authorized or made — that map records code-surface impact, and D124 changes no code surface, which is the same treatment D117, D118 and D120 through D123 received. src/disclosure_drift/m3/capacity_plan.py IS UNTOUCHED and no Decision 113 capacity constant moved. NOTHING WAS EXECUTED: no F1, no F2, no canary, no complete-source run, no E0, no network, no migration 0016, no database access, no governed SQLite writer, and no world was created, opened, modified, archived, copied, or deleted; nothing was written to the external SSD, which was a READ-ONLY DISCOVERY. THE D123 NUMBERING CONDITION IS RESOLVED PROSPECTIVELY RATHER THAN RETROACTIVELY: Decision 094 R70-R78 and Decision 123 R70-R78 are BOTH HISTORICALLY IMMUTABLE and NEITHER IS RENUMBERED (D124-G1), every future reference to a colliding ID must be DECISION-QUALIFIED (D124-G2), and from Decision 124 onward new owner rulings use DECISION-SCOPED IDS such as D124-R1 (D124-G3). TWO CONDITIONS ARE PUBLISHED RATHER THAN SILENTLY FIXED: the identifier c85744be921b0dc5 appears in NO TRACKED RECORD BEFORE DECISION 124 while Decision 059 section 3 names the older object, and reconciling that stored-object naming is a D125 obligation; and Decision 122 section 5 labels 638,071 COHORT-RESOLUTION ROWS while the D124 instruction reads it as explicit-resolution accessions, a term difference recorded rather than resolved because BOTH SECTION 7 QUOTIENTS HOLD UNDER EITHER READING. No owner-acceptance token was emitted by the publishing session.
NEXT_AUTHORIZED_ACTION: NEXT TECHNICAL AND OPERATIONAL STAGE IS D125 — ARCHIVAL PLUS VERIFIED INTERNAL RECLAMATION — AND IT IS NOT EXECUTED, NOT STARTED, AND NOT AUTHORIZED BY ANYTHING WRITTEN HERE. This entry records OWNER SEQUENCING ONLY. A D125 archival instrument must: hash the ACTUAL preserved D122 post-F1 working catalog, compare it to the historical candidate fa4a635d36a487774e02670bb0fab1ded1c696b5e25faf54fb6f55b69799f413 from the D123 completion report, STOP ON MISMATCH, and RECORD THE RESULTING IDENTITY BEFORE ANY DELETION (D124-R8) — that candidate is NOT YET ACCEPTED DURABLE IDENTITY; carry an explicit cryptographic/metadata manifest and/or a metadata-preserving container, because exFAT does not preserve all relevant POSIX metadata, covering relative paths, file types, exact bytes, SHA-256, original mode, original uid/gid or owner/group names where meaningful, run/world ID, repository HEAD/tree, external archive destination, and verification result (D124-R9); and reconcile the stored-object naming left open in Decision 124 section 4. ONLY AFTER EXTERNAL VERIFICATION may authorized INTERNAL copies be removed (D124-R7); C3's retained about 10 MiB evidence and tiny diagnostics REMAIN INTERNALLY. THE NEXT AUTHORIZED TECHNICAL ACTION IS NOT A COMPLETE-SOURCE RUN AND NOT E0. Both remain unauthorized, as do F1, F2, the full-population F2 rerun, the D117 retry, the three-source canary, the real replay proof, migration 0016, network, and acquisition. ACTIVE SQLITE USE OF THE EXTERNAL SSD IS NOT APPROVED and Plan B remains NOT APPROVED. DO NOT DELETE ANY WORLD OR RESIDUE ON THE STRENGTH OF D124: the D117, D120 and D122 worlds and the D123 characterization residue are ELIGIBLE UNDER A LATER INSTRUMENT and that instrument DOES NOT YET EXIST, and the single C3 working-catalog deletion is spent. Do not emit an owner-acceptance token — this session may publish an accepted ruling and may not accept anything.
