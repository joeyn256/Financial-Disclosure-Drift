...
NEXT_AUTHORIZED_ACTION: THE DECISION-081 FIXED COMPLETE-SUBMISSION-TEXT SOURCE VERIFICATION, EXECUTED ONCE UNDER Decision 081 §8, THEN RETURN TO SOL/GPT FOR OWNER ADJUDICATION — AND NOTHING ELSE. The six Decision-080 pending items are now OWNER-ADJUDICATED by accepted Decision 081 (2026-08-14, outcome M3_3_DECISION_080_SOURCE_ARCHITECTURE_OWNER_ACCEPTED), which freezes FIVE RULINGS. R46 MULTI-REGISTRANT RELATIONAL SEMANTICS: a genuinely multi-registrant accession does NOT possess a factual single registrant anchor merely because the current schema carries a scalar column; for exactly one substantive registrant the sole CIK MAY be the scalar registrant; for MORE THAN ONE, a primary CIK may NEVER be chosen by first-write order, minimum/maximum CIK, archive path, record order, hash, a submissions-document occurrence, or a filing-agent/submitter heuristic — WHICH REJECTS the Decision 080 §8.3 MR-3(a) intrinsic-submitter recommendation and does NOT adopt MR-3(c) blanket fail-closed exclusion either. EVERY substantive registrant association MUST BE REPRESENTED RELATIONALLY, the accession remains an ACCESSION-LEVEL OBJECT, and NO ARBITRARY SCALAR REGISTRANT MAY PARTICIPATE IN accession tie-break identity, candidate accession identity, selection identity, history assignment, or quota credit; where the scalar field cannot truthfully represent the filing it MUST become NULL/unresolved or otherwise cease to function as factual identity; the existing candidate registrant association layer SHOULD carry the full substantive registrant set and the top-level candidate_registrant_table_sha256 identity SHOULD carry the relational association content where compatible; a migration to represent this truthfully is AUTHORIZED IN PRINCIPLE and NOT IMPLEMENTED; and if OR1/R16 identity definitions require correction because they currently consume a false singleton registrant, the exact required change is RETURNED TO THE OWNER with NO REPLACEMENT SINGLETON INVENTED. R47 VERIFIED DOCUMENT-PURPOSE EVIDENCE: the Decision 080 §9 AP-1–AP-10 architecture is accepted IN PRINCIPLE under eleven required properties (protocol frozen before any adjudication document is read; frozen artifact SHA-256; exact source document identified; exact supporting span/location preserved; two independent OUTCOME-BLIND reviews; third adjudication or fail-closed on disagreement as frozen in the future execution protocol; frozen final adjudication; immutable provenance; no metadata overwrite; independent review of supporting spans; deterministic pipeline behavior AFTER the evidence table is frozen); the three frozen categories remain exactly administrative/certification/signature/exhibit-only, financial-statement/accounting/restatement/XBRL-correction, and narrative/business/risk/control/governance; keyword, substring, regex, LLM-only, filename, primaryDocDescription, operator-intuition, and form-suffix classification ALL REMAIN PROHIBITED; DECISION 081 PERFORMS ZERO CATEGORY CLASSIFICATIONS; and the future migration required because migration 0009 does not persist the 'verified' state is NOT AUTHORIZED. R48 VERIFIED EXPLICIT-ORIGINAL LINKAGE: a future authorized Complete Submission Text may establish amends_original at verified/document-level evidence ONLY when the amendment filing ITSELF explicitly identifies the original annual report by accepted compatible form 10-K or 10-KT, AND explicitly states either the exact original filing date OR the exact original accession, AND the assertion maps to EXACTLY ONE accepted catalog original under the same substantive registrant association with a compatible original form and the exact stated date or accession, AND no conflicting explicit statement exists, AND the accepted strict-later acceptance rule passes on authoritative accession-level acceptance evidence wherever ordering is required — this is EVIDENCE-BACKED RESOLUTION OF AN EXPLICIT FILING ASSERTION and is NOT date proximity, same-report-date inference, accession ordering, /A inference, name inference, or guessing; ZERO matches => unresolved, MULTIPLE => unresolved/review, CONFLICT => unresolved/review; Decision 018 co-selection and root requirements and the hard linked_amendment_entities quota 8 are UNCHANGED; and NO REAL ACCESSION IS RESOLVED. R49 E0 OWNER SEQUENCING: the Decision 080 §13 technical verdict E0_CAN_RUN_FAIL_CLOSED_BEFORE_ENRICHMENT is ACCEPTED, BUT M3.3-E0 REMAINS NOT AUTHORIZED UNTIL BOTH (A) the Decision-081 source-verification sample has returned AND Sol/GPT has adjudicated it, AND (B) the R46 multi-registrant bounded implementation correction has been implemented, independently reviewed, and owner-accepted — an OWNER SEQUENCING/SAFETY GATE preventing known false singleton registrant state from entering the first durable real parse, NOT a claim that E0 technically requires enrichment. R50 FIXED SOURCE-VERIFICATION SAMPLE AUTHORITY: ONE bounded stage over SEC EDGAR Complete Submission Text for sampled amendment accessions ONLY — NOT full-population acquisition — with TARGET_SAMPLE_N 125 maximum, LOGICAL_REQUEST_CEILING 125, PHYSICAL_ATTEMPT_CEILING 250, MAX 2 ATTEMPTS PER ACCESSION, at most 1 SEQUENTIAL request per second, NO parallel requests, NO crawler behavior, the already-accepted SEC identity/user-agent mechanism NEVER PRINTED, NO accession outside the frozen sample fetched, and NO redirect off sec.gov followed. THE VERIFICATION BOUNDARY IS FIXED BY Decision 081 §8: governance commit pushed and clean tree verified BEFORE any network authority is exercised; the accepted Decision-079 EPHEMERAL parse re-run SOLELY to reconstruct the frozen population and strata under Decision 079 R40/R41, creating NO E0 state, and RECONCILING to REAL_RAW_TOTAL_AMENDMENT_CANDIDATES 46912 and FROZEN_COHORT_AMENDMENT_CANDIDATES 20258 or STOPPING BEFORE NETWORK with M3_3_DECISION_081_SAMPLE_SELECTION_RECONCILIATION_FAILED; sampling ONLY from development / transition / primary_test / prospective / monitoring with outside_frozen_cohorts EXCLUDED and NO oversample rule overriding that exclusion; eligible forms EXACTLY 10-K/A and 10-KT/A; XBRL classes defined exactly as X0 has_xbrl==false, X1 has_xbrl==true AND has_inline_xbrl==false, X2 has_inline_xbrl==true, with any unknown class STOPPING the run before network; deterministic ranking within every stratum by ascending sha256("d081-source-verification/1.0:" + accession_plain) with NO stochastic step and NO body content, purpose, filing knowledge, or convenience selection; CORE 5 frozen cohorts × 3 XBRL classes × 6 accessions = target 90, then OVERSAMPLE A 10 10-KT/A, B 8 multi-registrant, C 8 multiple-compatible-original, D 8 zero-compatible-original, and E 1 missing-report-date if inside the frozen cohorts, each excluding already-selected accessions, TARGET MAXIMUM 125; an UNDERSIZED STRATUM TAKES ALL AVAILABLE MEMBERS with NO BACKFILL FROM ANOTHER STRATUM, so the final sample may be FEWER THAN 125; SAMPLE_ROWS, SAMPLE_ACCESSION_SET_SHA256, SAMPLE_PLAN_SHA256, and per-stratum counts FROZEN BEFORE THE FIRST NETWORK REQUEST and IMMUTABLE during execution, written ONLY to a NEW Decision-081 private run directory named m3-3-d081-source-verification-<hash-prefix> under the accepted external evidence root that writes into or alters NO accepted M3.2 artifact, catalog, or raw path; artifacts stored BYTE-FOR-BYTE with accession, source URL, retrieval timestamp, HTTP status, attempt number, byte length, and SHA-256, under a 5 GiB private aggregate ceiling and a 100 MiB single-artifact ceiling (OVERSIZE aborted while streaming and never retried merely for size), with NO absolute private path in any public output and NO already-acquired verification evidence deleted; request construction limited to the public SEC Complete Submission Text endpoint with NO EDGAR search/discovery query, NO index page first, ONE logical source object per sampled accession, TLS verification, and NO alternate mirror, CompanyFacts, Frames, separate XBRL endpoint, or second primary-document request; retry policy 200 no retry, 404 terminal absence with no retry unless URL construction is proven wrong first, 403/429 stop new requests + cool down at least 60 seconds + ONE retry with a second 403/429 STOPPING THE ENTIRE NETWORK STAGE, 5xx/transport timeout wait at least 5 seconds + ONE retry, and NO THIRD PHYSICAL ATTEMPT for any accession; measurements M1–M10 only; NO PURPOSE CLASSIFICATION — administrative/financial/narrative may NEVER be returned for any real accession, only explicit-purpose-statement present YES/NO with its preserved span, and NO QUOTA WITNESS IS CREATED; NO LINKAGE RESOLUTION — amends_original is NEVER written into any accepted data structure, only EXPLICIT_ORIGINAL_ASSERTION_PRESENT and ORIGINAL_LOOKUP_RESULT ZERO/EXACTLY_ONE/MULTIPLE are returned, granting NO linked-amendment quota credit; summary rates reported overall and by frozen stratum with cross-tabs by cohort, form, XBRL class, and multi-registrant oversample status, and NO extrapolation stated without being labeled an estimate; TOTALITY requiring every frozen sample accession to appear EXACTLY ONCE including failures and absences, with NO failing accession replaced, NO second sample generated, NO post-retrieval sample edit, and SAMPLE_TOTALITY returned; and CLOSEOUT at NETWORK_AUTHORIZATION = SPENT/CLOSED after the final request or any stop, with NO further SEC request under Decision 081, NO automatic enrichment, and NO "one more check". NO SESSION MAY BEGIN M3.3-E0 (the real offline metadata parse), M3.3-E1 (the real candidate snapshot and selection), M3.3-E2 (the real manifest and root), M3.4 (root approval), THE R46 MULTI-REGISTRANT IMPLEMENTATION CORRECTION, THE R47 EVIDENCE-SCHEMA MIGRATION, OR ANY ENRICHMENT ACQUISITION BEYOND THE FROZEN 125-ACCESSION SAMPLE ON ITS OWN AUTHORITY — EACH REMAINS A SEPARATE OWNER GATE THAT NO CONTRACT ACCEPTANCE, PASSING REHEARSAL, INDEPENDENT REVIEW, OWNER I/R ACCEPTANCE, GREEN SUITE, COMMIT, TAG, OR PRIVATE CATALOG SUPPLIES. M3_3_REAL_AMENDMENT_PURPOSE_FEASIBILITY_GATE_OPEN and M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE_OPEN are both ACTIVE and are never merged into one flag, and M3_3_REAL_ACCEPTANCE_ORDERING_ADEQUACY is PENDING FUTURE AUTHORIZED E0 VERIFICATION. Reacquisition and accepted-private-evidence WRITE authority remain NONE; migration remains none; and m3.2-complete remains unmoved
...
M3_3_SOURCE_VERIFICATION_SAMPLE_AUTHORIZATION: YES — ONE FIXED SAMPLE, CLOSED AFTER IT (accepted Decision 081 R50). Bounded network, SEC, and HTTP authority exists ONLY for the public SEC Complete Submission Text endpoint of the accessions in the frozen Decision-081 sample. LOGICAL_REQUEST_CEILING 125; PHYSICAL_ATTEMPT_CEILING 250; MAX 2 ATTEMPTS PER ACCESSION; at most 1 SEQUENTIAL request per second. NOT full-population acquisition, NOT reacquisition of any accepted M3.2 object, and NOT enrichment. The sample is frozen before the first request and is IMMUTABLE during execution; an undersized stratum takes all available members with NO cross-stratum backfill, so the final sample may be fewer than 125 and that is a correct outcome rather than a defect. After the final request or any stop, NETWORK_AUTHORIZATION = SPENT/CLOSED and no further SEC request may be made under Decision 081 — no automatic enrichment and no 'one more check'

M3_3_MULTI_REGISTRANT_CORRECTION_STATUS: REQUIRED BEFORE E0 — NOT AUTHORIZED AND NOT IMPLEMENTED IN THIS STAGE (accepted Decision 081 R46, R49). The 568 multi-registrant amendment accessions found by the Decision-079 audit have NO lawful single anchor, and every anchor-selection heuristic — first-write order, minimum/maximum CIK, archive path, record order, hash, a submissions-document occurrence, and any filing-agent or submitter heuristic — is PROHIBITED, including the intrinsic-submitter option Decision 080 §8.3 recommended as MR-3(a). The correction is a future bounded, owner-authorized stage requiring its own decision, implementation, independent review, and owner acceptance; a supporting migration is AUTHORIZED IN PRINCIPLE only. Until it lands, M3.3-E0 is blocked by R49 condition B

M3_3_VERIFIED_EVIDENCE_MIGRATION_STATUS: AUTHORIZED IN PRINCIPLE — NOT AUTHORIZED FOR IMPLEMENTATION AND NOT IMPLEMENTED (accepted Decision 081 R46, R47). Migration 0009 excludes 'verified' from every candidate evidence-level CHECK, so neither verified amendment-purpose evidence nor verified linkage evidence can be persisted under the current schema at all, and the relational multi-registrant representation may additionally require schema work. Migrations remain 0001-0013 and MIGRATION_AUTHORIZED remains none
