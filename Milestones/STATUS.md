...
CURRENT_STAGE: M3.3 — THE D137 INDEPENDENT REVIEW IS OWNER ACCEPTED AND ITS THREE MAJORS ARE CORRECTED AND PUBLISHED AS DECISION 138; NEITHER D137 NOR D138 IS OWNER ACCEPTED AND NEITHER AUTHORIZES A CANARY. The accepted verdict is D137_INDEPENDENT_REVIEW_CORRECTION_REQUIRED — ZERO BLOCKERS, THREE MAJORS — under review-acceptance token M3_3_D137_INDEPENDENT_IMPLEMENTATION_REVIEW_OWNER_ACCEPTED and correction authorization M3_3_D138_D137_SAFETY_ENVELOPE_CORRECTION_AUTHORIZED (issued outside this repository, now spent), classification BOUNDED_CORRECTION_ONLY, completion token M3_3_D138_D137_SAFETY_ENVELOPE_CORRECTION_COMPLETE_READY_FOR_REVIEW, AND NO ACCEPTANCE TOKEN. DECISION 137 REMAINS HISTORICAL AND IS NOT REWRITTEN TO PRETEND THE DEFECTS NEVER EXISTED; IT WAS NEVER OWNER ACCEPTED, AND D138 SUPPLIES NO ACCEPTANCE FOR IT AND CLAIMS NONE FOR ITSELF. THIS IS A BOUNDED CORRECTION, NOT A REDESIGN. MAJOR-1 — THE ENVELOPE COULD BE WALKED AROUND: require_volume_uuid was both the assertion AND the switch, so an operator who never typed --require-volume-uuid reached an unqualified disk, or the immutable D130 archive itself, WITH NO GUARD RUNNING AT ALL. THE DECISION NOW MOVES FROM THE ARGUMENT TO THE PATH (D138-R1): external_volume_candidate() classifies the RESOLVED root by DEVICE NUMBER on its NEAREST EXISTING ANCESTOR — so a world is classified before it is created and NO diskutil CALL IS MADE FOR AN INTERNAL ROOT — and require_external_envelope() is the SINGLE DECISION POINT: external runs the COMPLETE D137 envelope, internal-with-no-assertion returns None and is BYTE-FOR-BYTE THE ACCEPTED DECISION 116 PATH, and AN UNCLASSIFIABLE ROOT REFUSES because internal is the only answer that admits without proof. THE FLAG CAN NOW ONLY ADD A REQUIREMENT: it forces the envelope onto a root that would classify internal, and it must itself be 397A4D4A-9508-391E-814E-3B533C7BD049 — AN ARBITRARY UUID IS REFUSED BEFORE ANYTHING IS MEASURED, the expected identity handed to the envelope is the FROZEN CONSTANT and never the caller's string, and D138 CREATES NO GENERIC EXTERNAL-VOLUME AUTHORIZATION (D138-R12); D125-R4 REMAINS THE GENERAL RULE, narrowed only by D136-R8 and NOT WIDENED HERE. ARCHIVE EXCLUSION IS NOW UNCONDITIONAL (D138-R2) because the envelope is always entered; the refusal itself is BYTE-UNCHANGED D137-R3 realpath component comparison, so '..', a symlink and a case variant still cannot launder containment AND A LAWFUL SIBLING SHARING THE NAME PREFIX IS STILL NOT FALSELY REFUSED. ALL THREE ENTRY POINTS route through the one primitive, so a direct library caller cannot bypass what the CLI enforces and a diagnostic prefix cannot reach a volume the complete-source run would have refused. SQLITE_TMPDIR IS NOW VALIDATED WHERE SQLITE READS IT (D138-R3): the guard resolves the value from the PROCESS ENVIRONMENT and an explicitly supplied mapping must carry the IDENTICAL value — a disagreement is a REFUSAL, not a preference. MAJOR-2 — THE 10 GiB FLOOR WAS A PRINTED OPINION: classification plus an optional second process, and nothing else. ENFORCEMENT NOW LIVES INSIDE THE PROCESS RUNNING F2 (D138-R8): F2CapacityGuard samples from F2's OWN PER-ACCESSION LOOP IN BOTH TRAVERSALS and once BEFORE THE TRANSACTION OPENS, and raises F2CapacityHardStopError FROM INSIDE THE OPEN TRANSACTION so transaction() ROLLS IT BACK — the in-flight projection is DISCARDED, NOT TRUNCATED, and NO PARTIAL F2 ASSOCIATION STATE COMMITS. >20 GiB normal; <=20 GiB records a DURING_F2 alert and CONTINUES; <=10 GiB aborts; A CAPACITY-MEASUREMENT FAILURE TAKES THE SAME ABORT PATH, FAIL-CLOSED. BOTH THRESHOLDS REMAIN INCLUSIVE AND NEITHER MOVED (D124-R5, D135-R7). SAMPLING IS BOUNDED BY A MONOTONIC WALL CLOCK — 5 s SCHEDULED AGAINST A 60 s CEILING — NOT BY ITERATION COUNT, so a long batch cannot buy hours of unobserved execution, and a per-accession call stays affordable because most calls return immediately. NO THREAD, NO BACKGROUND PROCESS, NO SIGNAL, NO DELETION, NO ESCALATION — D131 UNTOUCHED. THE HARD-STOP EVIDENCE SURVIVES THE ROLLBACK (D138-R10): built BEFORE the raise and carried ON the exception — phase DURING_F2, reason, measured free bytes or the measurement-failure class, threshold, observation time, authenticated volume, and explicit f2_transaction_rolled_back: true / f2_committed: false. THE WATCHDOG IS DEMOTED (D138-R11) to supplemental operator observability: EXIT 6 DOES NOT STOP F2, and the module's prose, exit-code documentation and subcommand help all say so. THE GUARD BINDS ONLY ON THE PROTECTED EXTERNAL PATH; an internal run binds None. MAJOR-3 — TWO ACCEPTED GATES WERE NEVER WRITTEN. POST_F0_MINIMUM_FREE_BYTES = 64,424,509,440 (D138-R5) AND PRE_F1_MINIMUM_FREE_BYTES = 59,055,800,320 (D138-R6) ARE NOW EXECUTABLE, applied BEFORE F1 BEGINS; >= admits at each floor, one byte below refuses, AN UNMEASURABLE BOUNDARY REFUSES rather than being admitted, and NOTHING IS DELETED OR CLEANED TO CLEAR EITHER. They stay TWO SEPARATE NAMED GATES because they answer different questions. THE 185 GiB AND 50 GiB FLOORS KEEP THEIR OWN ACCEPTED CALL SITES AND ARE NOT MOVED (D138-R4, D138-R7), and the 30 GiB behaviour remains unreachable. VALIDATION: the accepted focused set was REPRODUCED UNCHANGED AT 384 PASSED — the exact D137 §17 count — and each review finding was reproduced as an executable assertion of the DEFECTIVE behaviour BEFORE ANYTHING WAS EDITED, in a temporary file deleted afterwards and never committed; 450 PASSED after (384 + 66, NO REGRESSION); ruff and ruff format --check clean; MYPY STRICT CLEAN OVER ALL 94 SOURCE FILES; 13 REVERSIBLE SOURCE MUTATIONS, EVERY ONE KILLED BY NAMED TESTS, every mutated file verified BYTE-IDENTICAL BY SHA-256 afterwards, NO SOURCE-MUTATING FRAMEWORK USED. TWO TEST WEAKNESSES WERE FOUND BY FALSIFICATION RATHER THAN BY DESIGN AND ARE RECORDED: an entry-reading test asserting on reading COUNT could not distinguish 'before F2 opens' from 'at F2's first accession' and now asserts on in_transaction; a sampling control asserting only samples > 1 survived removing either traversal's call and now asserts the SHAPE of what the readings observe. THE ROLLBACK PROOF IS NOT THAT AN EXCEPTION WAS RAISED: the capacity provider reads the association row count ON THE SAME CONNECTION from inside the open transaction, watches it CLIMB FROM ZERO, breaches at the first non-zero count, and the table is 0 AFTERWARDS — with a control run that COMMITS, so an empty table proves a rollback rather than a projection that never wrote. ONE BOUNDED READ-ONLY LIVE PREFLIGHT RAN: UUID EXACT MATCH; archive 24 ENTRIES, NO DIFFERENCES in 0.133 s with the 103,966,696,960-BYTE TAR stat-ed AND NEVER OPENED under an open tripwire; free 310,498,951,168 BYTES / 289.1747 GiB, BYTE-IDENTICAL to D137 §18, clearing the 185, 60 and 55 GiB floors; and FOUR NO-FLAG CONTROLS ALL REFUSED — external root, archive child, volume root, arbitrary asserted UUID — WHICH UNDER DECISION 137 WERE EVERY ONE OF THEM ADMISSIONS. An internal root with no flag returned None. NOTHING WAS CREATED, WRITTEN, DELETED OR BENCHMARKED ON THE VOLUME AND NO CANARY WORLD EXISTS; the ACCEPTING path remains proved SYNTHETICALLY because creating SQLITE_TMPDIR is a write. RECORDED ERRATUM: D137 §12's 'four of the twelve are mechanically verified and eight are not' is WRONG — the correct counts are FIVE AND SEVEN; the runbook now states them and D137 IS LEFT BYTE-UNCHANGED. NO MIGRATION: HEAD REMAINS 0015 AND 0016 IS ABSENT, UNAPPLIED AND NOT AUTHORIZED. ALL THREE ACTIVATION CONSTANTS REMAIN None. NETWORK REMAINS DISABLED at both tracked switches with request ceiling 0. THE D131 RUNTIME CONFIGURATION, PARSER AND SHARD-DISPATCH SEMANTICS ARE UNTOUCHED AND THE D134 mmap AND RELAXED-CHECKPOINT CANDIDATES REMAIN REJECTED. census_orchestrator.py::_parse_bulk REMAINS AN EXPLICIT PRE-NETWORK BLOCKER, DELIBERATELY UNREPAIRED. D129-R2, D129-R8 AND D129-R12 ARE NOT DISCHARGED. A COMPLETE-SOURCE CANARY IS NOT AUTHORIZED. E0 IS NOT AUTHORIZED.
ACTIVE_BLOCKER: THE CORRECTED COMPLETE-SOURCE CANARY REMAINS UNAUTHORIZED. D138's correction is COMPLETE but NOT INDEPENDENTLY REVIEWED AND NOT OWNER ACCEPTED, and D137 was never owner accepted either; the D136-R8 narrow one-canary exception takes effect ONLY AFTER D137 IMPLEMENTATION AND ACCEPTANCE, and correction alone moves nothing. A passing preflight still prints canary_authorized: false, which is a fact and not a formality. THE TWO D137 LIMITATIONS THAT WERE MAJORS ARE CLOSED — DURING_F2 IS NOW SAMPLED IN-PROCESS AND THE EXTERNAL REQUIREMENT IS NO LONGER OPT-IN — AND SEVEN RESIDUAL LIMITATIONS ARE CARRIED RATHER THAN CLOSED: (1) macos_volume_identity IS macOS-ONLY and fails closed elsewhere; (2) THE CLASSIFIER IS A MOUNT-POINT TEST, NOT A DEVICE-CLASS TEST — a disk image or network mount classifies as external and must then authenticate as the D136 volume, which is fail-closed but a broader net than 'USB disk'; (3) DURING_F2 SAMPLING IS BOUNDED, NOT INSTANTANEOUS — free space can fall below the floor and be observed up to one interval later, which is why 20 GiB of alerting sits above it; (4) THE SQLITE_TMPDIR GUARD VALIDATES PLACEMENT, NOT PEAK, and D136-R5 left peak spill unmeasured; (5) NO LIVE END-TO-END REHEARSAL WAS PERFORMED because that would require creating a world on the volume, so the envelope's ACCEPTING path is proved synthetically and by its live REFUSALS; (6) THE ROLLBACK PROOF RUNS ON A SMALL SYNTHETIC CATALOG — it proves the mechanism on tens of rows rather than tens of millions, and NO RUNTIME WAS MEASURED OR EXTRAPOLATED; (7) NOTHING HERE CHANGES ONE WORD OF THE D136-R6 PROCESS-CRASH-RECOVERY-ONLY BOUNDARY. SEPARATELY AND INDEPENDENTLY, census_orchestrator.py::_parse_bulk REMAINS AN OPEN PRE-NETWORK BLOCKER whose repair MUST NOT be performed as a side effect of unrelated work.
IMPLEMENTATION_AUTHORIZATION: SPENT — the D138 packet's authorization M3_3_D138_D137_SAFETY_ENVELOPE_CORRECTION_AUTHORIZED was issued outside this repository and is now consumed by this correction. NO FURTHER IMPLEMENTATION AUTHORITY ARISES FROM THE DECISION 138 PUBLICATION. It does NOT accept Decision 137, does NOT accept itself, does NOT adopt the external volume, does NOT create a canary world, a run identity, an execution namespace, a launch receipt, or a terminal state, does NOT authorize a corrected complete-source canary or any canary, does NOT authorize E0, does NOT create or authorize migration 0016, does NOT authorize network, SEC or HTTP at request ceiling 0, does NOT repair census_orchestrator.py::_parse_bulk, does NOT change D131 parser or runtime semantics, does NOT adopt any D134 candidate, does NOT widen the D136-R8 one-canary exception, and certifies NO semantic count. IT CLAIMS NO OWNER ACCEPTANCE. SELECTING A PATH, QUALIFYING A VOLUME, AND ADOPTING IT ARE THREE DISTINCT STEPS — D135 SELECTED, D136 QUALIFIED, D137 BUILT WHAT ADOPTION WILL NEED, AND D138 REPAIRED WHAT D137 BUILT — AND NONE OF THE FOUR TAKES IT.
NEXT_AUTHORIZED_ACTION: INDEPENDENT REVIEW OF THE DECISION 138 CORRECTION — AND NOTHING ELSE. It is not started and must not be started from this publication. A session that creates a canary world, a run identity, an execution namespace or a launch receipt, that writes to the SSK SSD, that sets any activation constant, that adds migration 0016, or that enables network on the strength of D137 or D138 is acting OUTSIDE both. THE SSD MUST NOT BE WRITTEN TO under this publication, the D130 archive MUST NOT BE MODIFIED, MOVED, DELETED OR HASHED beyond its four compact proofs, and the large D130 tar MUST NOT BE OPENED. After review, an OWNER DECISION ON WHETHER TO ACCEPT DECISION 137 AS CORRECTED BY DECISION 138 — and, separately and only then, whether to authorize the corrected canary.
