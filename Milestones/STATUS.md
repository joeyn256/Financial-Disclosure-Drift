...
IMPLEMENTATION_AUTHORIZATION: THE DECISION-085 CORRECTION OF THE FIVE ACCEPTED D083/D084 R46 FORMAL-REVIEW FINDINGS — M-1 AND MIN-1 THROUGH MIN-4 — AND NOTHING ELSE (accepted Decision 085 sections 3-9). Authorized paths are limited to what those five findings require: src/disclosure_drift/storage/migrations/0014_m33_multi_registrant_relational_correction.sql, src/disclosure_drift/m3/candidate_snapshot.py, src/disclosure_drift/sec/reserve_selector.py, the existing and new unit tests covering the candidate snapshot / R59 / MR-M10, migration 0014, the identity re-baselines and the reserve caps, audit or mutation tooling ONLY where the exact MR-M10 mutant requires it, and truthful current-state documentation. Broadening any path without owner action is PROHIBITED and is a STOP, and an unexpected path not clearly required by M-1 or MIN-1..MIN-4 is a STOP before editing. src/disclosure_drift/m3/candidate_identity.py remains PROHIBITED; acquisition.py and offline_execution.py need no further semantic change. ARCHITECTURE IS NOT REOPENED AND R58-R67 ARE NOT REDESIGNED. Migration 0014 remains PROSPECTIVE and PRE-E0: it is never applied to and never mutates the accepted private M3.2 operational catalog, it is exercised only through test, disposable, and repository migration machinery, and non-empty state requiring destructive reinterpretation is a STOP. PROHIBITED: cohorts.py; pilot_policy.py; migrations 0001-0013; preregistration.md; every existing record in Docs/Decisions/; network request logic; SEC transport behaviour; the D081 private verification evidence; the accepted M3.2 evidence root; migration 0015; the verified-evidence schema; and any document review or adjudication code. Required nonchange: SINGLE_REGISTRANT_UNEXPECTED_IDENTITY_DELTAS = 0, the affected governed identity inventory E1-E5 ONLY, snapshot_id / entity_tie_break_sha256 / the R15 evidence preimage / the R16 resolution preimage all unchanged, the R67 relational digest-binding proof still valid, no research quota value or declared domain changed, and no historical accepted decision rewritten. NO OTHER IMPLEMENTATION IS AUTHORIZED: migration 0015, the verified-evidence schema, REVIEW A, REVIEW B, the DOCUMENT ADJUDICATION, M3.3-E0, M3.3-E1, M3.3-E2, and M3.4 all remain UNAUTHORIZED, network, SEC, and HTTP authority is NONE with REQUEST_CEILING 0, and CORRECTION IS NOT ACCEPTANCE — R49 condition B is satisfied only after a fresh GENUINE Claude Fable 5 maximum review PASSES AND Sol/GPT accepts the corrected implementation
ACTIVE_STAGE_CONTRACT: Milestones/contracts/m3_3.md
NEXT_AUTHORIZED_ACTION: RETURN TO SOL/GPT FOR THE FINAL R46 OWNER-ACCEPTANCE DECISION ON THE PASSED GENUINE FABLE REREVIEW — AND NOTHING ELSE. The Decision-086-commissioned rereview is COMPLETE: a genuinely fresh Claude Fable 5 maximum epoch (harness identifier claude-fable-5, reported before substantive review; the model-identity gate PASSED with no Opus substitution) formally rereviewed the frozen corrected target 1c5b0150ecfc5e4695842e330d83f1ce2148c643 at tree 1994e8bfe54b8db03da765980f5df2d6dff822ba on 2026-08-15 and returned VERDICT PASS at BLOCKER 0 / MAJOR 0 / MINOR 0 / OPTIMIZATION 0 / OBSERVATION 3, token M3_3_D085_R46_GENUINE_FABLE_REREVIEW_PASSED_READY_FOR_OWNER_ACCEPTANCE, artifact Docs/m3/reviews/m3_3_d085_r46_genuine_fable_rereview_1c5b015.md. The rereview REVALIDATED EVERY FORMAL ACCEPTANCE PROPERTY, NOT ONLY THE DELTA: the correction is BOUNDED to M-1 and MIN-1 through MIN-4 plus truthful publication, with candidate_identity.py, candidate_snapshot.py, acquisition.py, and offline_execution.py blob-identical to 09ee4422; the exact MR-M10 derivation mutant was reproduced as SURVIVING all 207 builder-invoking tests of the original target and KILLED by exactly the new MR-M10A/Group-R59 builder protections on the corrected target with the real implementation passing (MR_M10_DERIVATION_MUTANT = KILLED, module provenance proven in-session for every mutant run, MR-M10A and MR-M10B present and non-redundant); MIN-2's four-door establishment invariant was proven on disposable catalogs over the full lifecycle A-N with the lawful E0 ingest shape executable WITHOUT another schema change; MIN-3's replacement before-literal 03e8736e… was reproduced from the genuine pre-correction parent 6fdec2ed's own fixture at chain head 0013 with UNVERIFIABLE_PRECORRECTION_DIGESTS = 0 and the false 5f3f6a57… literal absent from every test and source file; MIN-4's reserve per-CIK cap charges every truthful substantive registrant (including a three-registrant case) with accession-domain totals counting one joint filing once and an absent-from-pool bundle failing closed; MR-M1 through MR-M14 are ALL EFFECTIVE with first-member-anchor and membership-dependent-slot mutants additionally KILLED and historical M20/M22 re-executed KILLED/KILLED at 38/38 anchors; the R67 binding is TRUE (remove/change/add moves E3->E4->E5 through the real selection-identity builder, reorder does not); SINGLE_REGISTRANT_UNEXPECTED_IDENTITY_DELTAS = 0 authenticated against the genuine pre-correction rule plus a non-pinned case; R58/R59/R60 are enforced with no reachable fabricated primary; the identity impact is exactly E1-E5; R62 attribution shows no accession-domain double counting and the hard multi-registrant quota is unchanged at 2 accession-keyed; R65 is constant-only and R66 caller-only; E1-E8 are 8/8 PASS with write-free replay; migration 0014 sits at position 0014 with 0001-0013 byte-identical to the accepted baseline, fresh-build and upgrade schemas byte-identical over 225 objects, foreign_key_check and integrity_check clean, the empty-state guard operational, and the provenance chain recognizing the corrected bytes; R68 was measured EXACTLY AS ACCEPTED — of the fixture's eight manifest components only selector_policy_sha256 moved (29783a60… -> cd237060…) with root_manifest_sha256 and manifest_id moving as derived identities, the other seven components byte-identical including candidate_tables_sha256, selection_result_sha256 unchanged, and the canonical-JSON length unchanged, with NO unclaimed component moving; and ONE routine make check-fast returned exit 0 with the 16-file targeted battery at 1568 passed / 1 pre-existing skip. THE REREVIEW CORRECTED NOTHING AND GRANTS NOTHING: R49 CONDITION B IS NOT YET SATISFIED — a PASS is only its first half, and it becomes satisfied ONLY when Sol/GPT explicitly owner-accepts the corrected R46 implementation. UNTIL THAT ACCEPTANCE: M3.3-E0, M3.3-E1, M3.3-E2, AND M3.4 REMAIN UNAUTHORIZED; MIGRATION 0015, REVIEW A, REVIEW B, AND THE DOCUMENT ADJUDICATION REMAIN UNAUTHORIZED; M3_3_REAL_AMENDMENT_PURPOSE_FEASIBILITY_GATE_OPEN and M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE_OPEN both remain ACTIVE and are never merged into one flag, and M3_3_REAL_ACCEPTANCE_ORDERING_ADEQUACY is PENDING FUTURE AUTHORIZED E0 VERIFICATION. NETWORK, SEC, AND HTTP AUTHORITY IS NONE WITH REQUEST_CEILING 0; MIGRATION_AUTHORIZED is NONE (the 0014 correction is complete and committed) with 0015 ABSENT; and m3.2-complete remains unmoved with NO tag created
...
M3_3_MULTI_REGISTRANT_CORRECTION_STATUS: OWNER ACCEPTED AND AUTHORIZED FOR IMPLEMENTATION — MIGRATION 0014 ONLY (accepted Decision 081 R46, R49; contract at accepted Decision 082 section 10; owner acceptance and the R58-R62 adjudications at accepted Decision 083). The 568 multi-registrant amendment accessions found by the Decision-079 audit have NO lawful single anchor, and every anchor-selection heuristic — first-write order, minimum/maximum CIK, archive path, record order, hash, a submissions-document occurrence, and any filing-agent or submitter heuristic — remains PROHIBITED, including the intrinsic-submitter option Decision 080 section 8.3 recommended as MR-3(a). ALL FIVE Decision 082 section 10.15 OPEN ITEMS ARE NOW ADJUDICATED BY DECISION 083: (1) the NEW census_accession_registrants relation is ADOPTED and is authoritative (R58); (2) the anchor-slot filler is OPTION H-a, the exact non-CIK sentinel MULTI_REGISTRANT_NO_SINGLETON, used only for an ESTABLISHED set of cardinality >1 and never persisted in a CIK column (R60); (3) Decision 021 manifest ITEM 48 'anchor CIK' becomes PROSPECTIVELY NULLABLE — the factual CIK at established cardinality 1 and NULL above it — WITHOUT rewriting Decision 021 and with NO fabricated replacement anchor (R61); (4) history and event attribution reaches EVERY substantive registrant, while accession-domain calculations still deduplicate one joint filing as ONE accession and no quota changes its declared domain (R62); and (5) registrant_set_completeness = unestablished BLOCKS ACCESSION CANDIDACY ENTIRELY, fail-closed with an explicit accepted reason, rather than blocking only the scalar anchor (R59). The implementation is bounded to Decision 082 section 10.14's path set plus current-state documentation; migration 0014 is PROSPECTIVE and PRE-E0 and never mutates the accepted private M3.2 operational catalog, with non-empty state requiring destructive reinterpretation a STOP; historical Decision-070-077 rehearsal evidence is IMMUTABLE and affected synthetic scenarios receive NEW correction-stage expectations; SINGLE_REGISTRANT_UNEXPECTED_IDENTITY_DELTAS = 0 is required; and all FOURTEEN MR-M1 through MR-M14 protections are implemented at their exact definitions with DEMONSTRATED effectiveness. Successful implementation is NOT acceptance: R49 condition B additionally requires a fresh independent review and Sol/GPT owner acceptance, and until that lands M3.3-E0 remains blocked

M3_3_VERIFIED_EVIDENCE_MIGRATION_STATUS: OWNER ACCEPTED — IMPLEMENTATION DEFERRED; MIGRATION 0015 NOT AUTHORIZED AND NOT IMPLEMENTED (accepted Decision 081 R46, R47; contract at accepted Decision 082 section 11; owner acceptance and the four dispositions at accepted Decision 083 R63). Migration 0009 excludes 'verified' from every candidate evidence-level CHECK by design, so neither verified amendment-purpose evidence nor verified linkage evidence can be persisted under the current schema at all, and amendment_purpose_quota_eligible additionally requires amendment_purpose_evidence_level = 'provisional'. Decision 082 section 11 states the minimum design: four new append-only relations (document_artifacts, document_review_records, document_review_spans, document_adjudicated_evidence) plus the CHECK widening, at PROPOSED MIGRATION 0015, SEPARATE FROM and ORDERED AFTER the R46 migration 0014 so the evidence layer is never built on a schema that still asserts a false singleton. THE EXACT IDENTITY IMPLICATION IS FROZEN: hash_table hashes the table name, the column tuple, and the sorted rendered rows with NULL rendered as a fixed sentinel, so ADDING A COLUMN TO AN EXISTING DIGEST TUPLE CHANGES THAT DIGEST FOR EVERY ROW EVEN WHEN THE COLUMN IS NULL EVERYWHERE, while a NEW DOMAIN leaves every existing digest byte-unchanged — therefore the evidence layer may be added with ZERO disturbance to accepted synthetic I/R identities IF AND ONLY IF it lives entirely in NEW hashing domains and NO existing column tuple is widened, and no second hash implementation is introduced. THE FOUR OPEN OWNER ITEMS ARE NOW ADJUDICATED BY DECISION 083 R63: document_artifacts is a CATALOG METADATA relation with the artifact bytes remaining in the private external evidence root, NO absolute private filesystem path persisted, and NO EV_ROOT exposure; amendment_linkage_state = 'amends_original' is REUSED with verification strength carried by evidence_level = 'verified' and NO second semantic state such as verified_amends_original invented; verified evidence is authorized in M3.3 v1 ONLY for amendment purpose and amendment linkage / explicit-original evidence, with the future migration and policy validation REQUIRED TO ENFORCE that applicability and never silently enabling size, industry, history, universe, cohort, XBRL, or control predicates; and reviewer identity is a durable OPAQUE review-epoch identifier plus reviewer role and model, with NO personal name persisted and NO raw Claude session ID required, the package mechanically distinguishing Review A, Review B, and adjudication epochs. Migration 0015 remains NOT AUTHORIZED
