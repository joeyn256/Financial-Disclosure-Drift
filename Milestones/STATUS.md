...
CURRENT_STAGE: M3.3 — THE D136 EXTERNAL SSD ACTIVE-WORKING-VOLUME QUALIFICATION IS ACCEPTED AND PUBLISHED; THE EXACT SSK SSD IS MECHANICALLY QUALIFIED; A NARROW ONE-CANARY ExFAT / SHARED-FAILURE-DOMAIN EXCEPTION IS SELECTED; D137 SAFETY/PATH IMPLEMENTATION IS NEXT. Decision 136 is ACCEPTED under acceptance token M3_3_D136_QUALIFICATION_EVIDENCE_OWNER_ACCEPTED, numeric acceptance token M3_3_D136_D135_NUMERIC_AUTHENTICATION_OWNER_ACCEPTED, and architecture ruling token M3_3_D136_SINGLE_CANARY_EXTERNAL_SSD_EXCEPTION_SELECTED; classification BOUNDED_VOLUME_QUALIFICATION_ONLY, verdict D136_EXTERNAL_SSD_PASS_WITH_ARCHIVE_ISOLATION_REQUIRED. THE QUALIFICATION D135 §10 (D135-R5) REQUIRED BEFORE ANY SSD COULD BE ASSUMED SUITABLE NOW EXISTS — IT COMPLETES THE SECOND OF D135'S THREE STEPS, AND QUALIFICATION IS NOT ADOPTION. D136 SHIPS AN EMPTY EXECUTABLE CHANGE SET: no source, test, script, configuration, schema, or migration byte moved for it. QUALIFIED VOLUME (D136-R1): /Volumes/SSK SSD, VOLUME UUID 397A4D4A-9508-391E-814E-3B533C7BD049, /dev/disk4s2 on physical /dev/disk4, ExFAT VIA APPLE FSKit (user-space framework, not a kernel VFS driver), USB, External, Solid State, 131,072 B allocation block, SMART NOT SUPPORTED; exactly one external volume was present so the identity was unambiguous, and NO REJECTION CRITERION WAS TRIGGERED. THE STABLE IDENTITY IS THE VOLUME UUID, NEVER THE ATTACH-TIME disk4s2. CAPACITY PASSES (D136-R2): post-cleanup free 310,498,557,952 BYTES / 289.1743 GiB against the START floor 198,642,237,440 BYTES / 185 GiB — SURPLUS 111,856,320,512 BYTES / 104.1743 GiB; headroom 141.4218 GiB at projected peak and 161.4302 GiB at projected final; three pre-write and three post-cleanup observations each spread 0 BYTES; NO ARCHIVED BYTE COUNTED AS FREE; THE 185 GiB START AND 50 GiB PRE-F2 FLOORS ARE UNCHANGED AND STILL CONTROLLING. SQLITE MECHANICS PASS (D136-R3) in the project's OWN environment (.venv, Python 3.12.13, SQLite 3.53.4, NOT system 3.9.5/3.35.5) WITH NO PROJECT CODE ALTERED: journal_mode=wal, synchronous=2, cache_size=-524288, wal_autocheckpoint=1000, foreign_keys=1, busy_timeout=10000, mmap_size=0; LOCKING PROVED WITH SEPARATE OS PROCESSES — writer B refused its full 2.1759 s bounded timeout with 'database is locked' AND LEFT NO ROW, a reader was NEVER BLOCKED (0.0005 s) with correct WAL snapshot isolation, B committed in 0.003 s after release, NO DOUBLE WRITER; CRASH RECOVERY CORRECT BOTH WAYS under a TEN-CHECK PID-AND-COMMAND-LINE AUTHENTICATION IMMEDIATELY BEFORE EACH SIGKILL (no escalation ladder, no project or canary process signalled) — 500 uncommitted rows ABSENT after the kill, 500 committed rows SURVIVED a kill with a 24,752 B UNCHECKPOINTED WAL and were recovered by a FRESH PROCESS, TRUNCATE returned [0,0,0], integrity_check == ok throughout. I/O PRACTICALITY PASSES (D136-R4): 2 GiB non-sparse with write-hash == read-back-hash, 250.438 MiB/s FSYNC-COMPLETE against the 50 MiB/s floor (5x), 322.438 MiB/s read, 0 I/O ERRORS; bounded SYNTHETIC SQLite — D134 NOT RECONSTRUCTED, NO SEC JSON PARSED, NO CANARY RUN — built 793.96 MiB in 4.176 s with commit latency median 0.0388 s / p95 0.0544 s / max 0.0741 s, WAL high-water 8.927 MiB, 0 STALLS OVER 5 s, 0 I/O ERRORS, integrity_check == ok, WAL truncated to 0; A FLAT WAL SAMPLE IS NOT ABSENCE OF CHECKPOINTING — SQLite reuses the WAL in place, and the proof it ran is that the high-water held at 8.927 MiB WHILE 793.96 MiB WAS WRITTEN; NO COMPLETE-SOURCE RUNTIME MAY BE EXTRAPOLATED. ALLOCATION (D136-R5): external ratios 1.000000 (2 GiB file) and 1.0000443 (SQLite DB) are BETTER THAN THE INTERNAL APFS 1.0002757 the floor was derived on; rescaling adds only 6,073,630 B durable and 7,024,938 B peak; NO FLOOR RAISED AND NO ADJUSTMENT PROPOSED; the 128 KiB minimum allocation is a KNOWN MINOR many-small-file risk (an 8,192 B DB allocated 131,072 B, 16x) bounded because the run world is a few very large objects — 853,404 FILES WOULD BE NEEDED TO CONSUME THE SURPLUS — with SQLITE_TMPDIR BEHAVIOUR UNMEASURED and carried to D137. D136 ESTABLISHES PROCESS-CRASH RECOVERY ONLY (D136-R6): NO POWER-LOSS SAFETY, NO SURPRISE-REMOVAL SAFETY, NO USB-BRIDGE CACHE-FLUSH CORRECTNESS, NO JOURNALED-FILESYSTEM SEMANTICS; ExFAT HAS NO METADATA JOURNAL; the ~0.9 ms F_FULLFSYNC return is consistent with a satisfied flush AND EQUALLY WITH THE BRIDGE IGNORING IT, and D136 cannot distinguish them without power-cut testing. D130 ARCHIVE STABLE AND UNIQUELY RETAINED (D136-R7): the ~104 GB pax archive was NOT OPENED, NOT HASHED, NOT RE-HASHED; only the D130 §6 compact proofs were verified (0.040 s) and the postcheck found ZERO DIFFERENCES ACROSS ALL 24 ENTRIES with the archive directory NEVER WRITTEN; the scratch tree was a SIBLING at the volume root; d130_post_deletion_proof.txt shows NO INTERNAL COPY EXISTS, so archive and any future active database share FILESYSTEM, PHYSICAL DEVICE, CABLE/BRIDGE AND SURPRISE-REMOVAL FAILURE DOMAIN — which is why the verdict is CONDITIONAL, NOT SOFTENED FROM A FAIL (no predicate failed) AND NOT INCONCLUSIVE (testing was complete). D135 NUMERIC RULING (D136 §3): THE AUTHORITATIVE ROW-UPLIFT NUMERATOR IS 21,098,678, from 15,996,591 + 5,102,087, giving U_rows ~= 1.3189484; THE 21,099,278 VALUE WAS AN OWNER PACKET TYPO and appears in NO repository or evidence file; all four required sources agree, capacity_model_locked.json hash-authenticates against the cba5ba2b...b69a982 D135 ITSELF PUBLISHED, and re-running the accepted bc cross-check reproduces EVERY published total exactly while the alternative reproduces NONE; the blocking token was CORRECTLY NOT RETURNED because it names a mismatch between publication and locked model and NO SUCH MISMATCH EXISTS; NO REPOSITORY CORRECTION IS REQUIRED OR AUTHORIZED. D125-R4 REMAINS THE GENERAL RULE — SSK SSD cold/archive only, no active governed SQLite use, no reformat — NARROWED BY ONE EXCEPTION (D136-R8): after successful D137 implementation and acceptance, THIS EXACT VOLUME may host ONE corrected M3.3 complete-source canary working world, as a SEPARATE SIBLING TREE, with NO WRITE INSIDE /Volumes/SSK SSD/FDD_M3_3_D130_D128_ARCHIVE and THE D130 ARCHIVE IMMUTABLE; it authorizes NO arbitrary active governed SQLite use and GRANTS NO AUTHORITY NOW. THE SHARED FAILURE-DOMAIN RISK IS EXPLICITLY ACCEPTED FOR THAT ONE FUTURE CANARY ONLY (D136-R9) on five stated grounds, and THIS CONVERTS ExFAT INTO NOTHING AND CLAIMS ZERO RISK NOWHERE. TWELVE PHYSICAL/RUN CONDITIONS (D136-R10) bind any later canary authorization and are FUTURE RUN CONDITIONS ONLY. THE 30 GiB CONSTANT REMAINS LIVE IN CODE EXACTLY AS WRITTEN — PRE_F2_MINIMUM_FREE_BYTES in src/disclosure_drift/m3/single_source_canary.py IS NOT EDITED AND NOT AUTHORIZED TO BE EDITED; replacing its behaviour is D137 WORK (D136-R11 item 6). D129-R2 REMAINS CONTROLLING and D129-R8 REMAINS CONTROLLING — FROM SCRATCH, NEW WORLD, NEW RUN ID (D136-R12). THE D131 RUNTIME CONFIGURATION IS UNCHANGED AND THE D134 mmap AND RELAXED-CHECKPOINT CANDIDATES REMAIN REJECTED. MIGRATION HEAD REMAINS 0015 AND 0016 IS ABSENT, UNAPPLIED AND NOT AUTHORIZED. ALL THREE ACTIVATION CONSTANTS REMAIN None. NETWORK REMAINS DISABLED at both tracked switches with request ceiling 0. census_orchestrator.py::_parse_bulk REMAINS AN EXPLICIT PRE-NETWORK BLOCKER, DELIBERATELY UNREPAIRED. A COMPLETE-SOURCE CANARY IS NOT AUTHORIZED. E0 IS NOT AUTHORIZED.
ACTIVE_BLOCKER: THE CORRECTED COMPLETE-SOURCE CANARY REMAINS UNAUTHORIZED UNTIL THE D137 EXTERNAL WORKING-ROOT / IDENTITY-GUARD / 50 GiB PRE-F2 IMPLEMENTATION IS COMPLETED, INDEPENDENTLY REVIEWED, AND OWNER ACCEPTED. D136 qualified the volume mechanically and created a narrow one-canary exception to D125-R4, but QUALIFICATION IS NOT ADOPTION and the exception GRANTS NO AUTHORITY NOW: it takes effect only after successful D137 implementation and acceptance. D137 must deliver all nine D136-R11 items — external working-root selection (existing supported surface if one exists, otherwise the SMALLEST NEW SURFACE NECESSARY); a FAIL-CLOSED identity guard keyed on the STABLE VOLUME UUID 397A4D4A-9508-391E-814E-3B533C7BD049 rather than the attach-time disk4s2; REFUSAL IF THE WORKING ROOT IS INSIDE THE D130 ARCHIVE TREE; a >= 185 GiB / 198,642,237,440 BYTE launch floor; a >= 50 GiB / 53,687,091,200 BYTE PRE-F2 floor; REPLACEMENT OF THE KNOWN-INADEQUATE 30 GiB PRE_F2_MINIMUM_FREE_BYTES BEHAVIOUR; D135-R7-consistent phase-boundary and F2 capacity monitoring with the DURING_F2 hard floor UNCHANGED AT 10 GiB (D124-R5) and the operator told explicitly that BECAUSE F2 IS A SINGLE TRANSACTION A STOP IS A ROLLBACK; operator/runbook requirements for external power, stable mount and caffeinate/no-sleep execution; and TARGETED TESTS WITH INDEPENDENT VALIDATION. D137 DOES NOT START THE CANARY. SEPARATELY AND INDEPENDENTLY, census_orchestrator.py::_parse_bulk REMAINS AN OPEN PRE-NETWORK BLOCKER whose repair MUST NOT be performed as a side effect of unrelated work.
IMPLEMENTATION_AUTHORIZATION: NONE — AND SPECIFICALLY NONE FROM THE DECISION 136 PUBLICATION. D136 IS A GOVERNANCE RECORD OF A COMPLETED BOUNDED QUALIFICATION PLUS AN ARCHITECTURE EXCEPTION THAT TAKES EFFECT ONLY LATER: its executable change set is EMPTY, and it adopts nothing. IT DOES NOT ADOPT THE EXTERNAL VOLUME AS THE WORKING ROOT, does not change path configuration, does not add external-root support, DOES NOT EDIT PRE_F2_MINIMUM_FREE_BYTES (still 30 * 1024**3 and NOT AUTHORIZED TO BE EDITED HERE), does not change the 185 GiB or 50 GiB floors, does not create or authorize migration 0016, does not authorize E0, network, SEC or HTTP at request ceiling 0, does not repair census_orchestrator.py::_parse_bulk, certifies NO semantic count, and discharges NEITHER D129-R2, D129-R8 NOR D129-R12. IT CLAIMS NO POWER-LOSS SAFETY AND EXTRAPOLATES NO CANARY RUNTIME. SELECTING A PATH, QUALIFYING A VOLUME, AND ADOPTING IT ARE THREE DISTINCT STEPS — D136 COMPLETES ONLY THE SECOND.
NEXT_AUTHORIZED_ACTION: AN OWNER-PREPARED DECISION 137 IMPLEMENTATION PACKET — AND NOTHING ELSE. IT IS NOT STARTED AND MUST NOT BE STARTED FROM THIS PUBLICATION: no implementation authority arises from it. A session that begins external working-root support, writes the volume-identity guard, edits PRE_F2_MINIMUM_FREE_BYTES, or touches the SSD on the strength of D136 alone is acting OUTSIDE D136. THE SSD MUST NOT BE WRITTEN TO under this publication, D136 MUST NOT BE RERUN, its scratch directory MUST NOT BE RECREATED, and the large D130 tar MUST NOT BE TOUCHED OR HASHED.
