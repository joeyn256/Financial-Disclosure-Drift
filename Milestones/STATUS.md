...
IMPLEMENTATION_AUTHORIZATION: THE R46 MULTI-REGISTRANT RELATIONAL CORRECTION AND MIGRATION 0014 ONLY (accepted Decision 083 section 10; contract at accepted Decision 082 section 10 as modified by R58-R62). Authorized paths are EXACTLY Decision 082 section 10.14's set plus only the current-state documentation needed to report implementation completion truthfully; broadening that set without owner action is PROHIBITED. Migration 0014 is PROSPECTIVE and PRE-E0: it is never applied to and never mutates the accepted private M3.2 operational catalog, it is exercised only through test, disposable, and repository migration machinery, and non-empty state requiring destructive reinterpretation is a STOP. PROHIBITED: cohorts.py; pilot_policy.py; migrations 0001-0013; preregistration.md; every existing record in Docs/Decisions/; every network, acquisition, and transport module; the D081 private verification evidence; the accepted M3.2 evidence root; migration 0015; the verified-evidence schema; and any document review or adjudication code. NO OTHER IMPLEMENTATION IS AUTHORIZED: migration 0015, the verified-evidence schema, REVIEW A, REVIEW B, the DOCUMENT ADJUDICATION, M3.3-E0, M3.3-E1, M3.3-E2, and M3.4 all remain UNAUTHORIZED, network, SEC, and HTTP authority is NONE with REQUEST_CEILING 0, and SUCCESSFUL IMPLEMENTATION IS NOT ACCEPTANCE — R49 condition B is satisfied only after a fresh independent review AND Sol/GPT owner acceptance
ACTIVE_STAGE_CONTRACT: Milestones/contracts/m3_3.md
NEXT_AUTHORIZED_ACTION: IMPLEMENT THE R46 MULTI-REGISTRANT RELATIONAL CORRECTION AND MIGRATION 0014 UNDER ACCEPTED DECISION 083 SECTION 10 — AND NOTHING ELSE — THEN RETURN TO SOL/GPT. Accepted Decision 083 (2026-08-14, outcome M3_3_DECISION_082_PRE_E0_CONTRACTS_OWNER_ACCEPTED) accepts ALL THREE Decision-082 contracts, treats the pushed Decision-082 commit 5231359f as the SOLE Decision-082 execution with the duplicate-delivery condition CLOSED, adjudicates every open item those contracts left open by freezing R58-R64, and authorizes EXACTLY ONE bounded implementation: the R46 correction and migration 0014 under Decision 082 section 10.14's path set, plus only the current-state documentation needed to report completion truthfully. THE AUTHORIZED SEMANTICS ARE FIXED. R58: the new census_accession_registrants relation is AUTHORITATIVE; the scalar registrant field may carry a canonical CIK only for an ESTABLISHED set of cardinality exactly 1 and is NULL above it; and every anchor-selection heuristic — first write, last write, minimum CIK, maximum CIK, archive order, record order, hash order, submissions occurrence, full-index row order, submitter, filing agent, transport URL, filename — is PROHIBITED. R59: registrant_set_completeness = unestablished BLOCKS ACCESSION CANDIDACY ENTIRELY, fail-closed with an explicit accepted reason, and is NEVER read as evidence of a sole registrant. R60: the exact non-CIK sentinel MULTI_REGISTRANT_NO_SINGLETON may occupy the tie-break registrant slot ONLY for an ESTABLISHED set of cardinality >1 and is never a CIK, never persisted in a CIK column, never an entity, never counted toward an entity or quota, and never a transport locator; ESTABLISHED SINGLE-REGISTRANT PREIMAGES MUST REMAIN BYTE-FOR-BYTE IDENTICAL; and unestablished sets hash no fake value at all. R61: Decision 021 is NOT rewritten, manifest ITEM 48 'anchor CIK' becomes prospectively NULL for an established multi-registrant accession with NO fabricated replacement anchor, candidate_registrant_table_sha256 binds the relational association set, the FIVE identity consumers E1-E5 are accepted as prospectively changeable before real E0, and snapshot_id, entity_tie_break_sha256, the R15 evidence preimage, and the R16 resolution preimage are preserved as unaffected — IF IMPLEMENTATION PROVES THAT FIVE-ITEM INVENTORY INCOMPLETE THE SESSION STOPS AND RETURNS TO SOL/GPT. R62: history and event attribution reaches EVERY substantive registrant, accession-domain calculations still deduplicate one joint filing as ONE accession, entity-domain metrics admit each truthful substantive entity under their EXISTING definitions, no quota changes its declared domain, and Decision 072's hard multi-registrant quota of 2 with its accession-keyed witness is unchanged. MIGRATION 0014 IS PROSPECTIVE AND PRE-E0: it is never applied to and never mutates the accepted private M3.2 operational catalog, it is exercised only through test, disposable, and repository migration machinery, and non-empty state requiring destructive reinterpretation is a STOP. HISTORICAL DECISION-070-077 REHEARSAL AND EVIDENCE ARTIFACTS ARE IMMUTABLE and are never rewritten because corrected prospective hashes differ; affected synthetic scenarios receive NEW correction-stage expectations with every changed hash traceable exclusively to R46 / R58-R62 semantics; and SINGLE_REGISTRANT_UNEXPECTED_IDENTITY_DELTAS = 0 is required. All FOURTEEN Decision 082 section 10.13 protections MR-M1 through MR-M14 are implemented at their exact definitions, NOT reduced to representative coverage, with effectiveness DEMONSTRATED rather than assumed. PROHIBITED FOR THIS IMPLEMENTATION: cohorts.py; pilot_policy.py; migrations 0001-0013; preregistration.md; every existing record in Docs/Decisions/; every network, acquisition, and transport module; the D081 private verification evidence; the accepted M3.2 evidence root; migration 0015; the verified-evidence schema; and any document review or adjudication code. R63 accepts the verified-evidence schema contract with IMPLEMENTATION DEFERRED and MIGRATION 0015 NOT AUTHORIZED, and R64 accepts the document-adjudication protocol with EXECUTION DEFERRED and REVIEW A, REVIEW B, and THE ADJUDICATION ALL NOT AUTHORIZED. EVEN AFTER SUCCESSFUL IMPLEMENTATION, M3.3-E0, M3.3-E1, M3.3-E2, and M3.4 REMAIN UNAUTHORIZED and R49 condition B is satisfied ONLY after the implementation receives a fresh independent review AND Sol/GPT owner acceptance — the implementing session does NOT self-review for formal acceptance. M3_3_REAL_AMENDMENT_PURPOSE_FEASIBILITY_GATE_OPEN and M3_3_REAL_LINKED_AMENDMENT_FEASIBILITY_GATE_OPEN both remain ACTIVE and are never merged into one flag, and M3_3_REAL_ACCEPTANCE_ORDERING_ADEQUACY is PENDING FUTURE AUTHORIZED E0 VERIFICATION. NETWORK, SEC, AND HTTP AUTHORITY IS NONE WITH REQUEST_CEILING 0; reacquisition and accepted-private-evidence WRITE authority remain NONE; MIGRATION_AUTHORIZED is 0014 ONLY; and m3.2-complete remains unmoved with NO tag created
