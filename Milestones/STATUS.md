...
> incomplete, no `m3.2-complete` tag, no live acquisition occurred, or M3.2B outstanding is
> HISTORICAL** — accurate as at its own record's acceptance, and superseded as a statement of
> current state. The machine-readable markers at the end of this file carry the current position.
...
NEXT_AUTHORIZED_ACTION: SOL/GPT OWNER RULINGS ON OR-1 AND OR-2 — the only two owner rulings still open, and both ENTRY-BLOCKING. The M3.3 entry and contract packets have been ISSUED (M3.3-G and M3.3-GR, both 2026-08-13), so that clause is DISCHARGED; the CONTRACT-ACCEPTANCE clause STILL BINDS. Docs/m3/m3_3_snapshot_authority_adjudication_proposal.md is a PROPOSAL WITH NO AUTHORITY and is NOT a ruling. Only after the OR-1 and OR-2 rulings are frozen may a Decision 067 and a corrected M3.3 contract be prepared for FRESH INDEPENDENT CONTRACT REVIEW, followed by owner adoption of any required corrections, a fresh rereview, and owner acceptance. M3.3-I/R REMAINS NOT AUTHORIZED AND NO M3.3 WORK MAY BEGIN
