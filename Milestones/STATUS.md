...
CURRENT_STAGE: M3.3 PRE-E0 — THE FINAL LIVE COMPLETE-SOURCE PREFLIGHT IS ACCEPTED (D126) AND THE ONE BLOCKER IT FOUND IS NOW IMPLEMENTED AND AWAITING INDEPENDENT REVIEW (D127). D126 ACCEPTED (M3_3_D126_COMPLETE_SOURCE_PREFLIGHT_OWNER_ACCEPTED), published as a RETROSPECTIVE DURABLE RECORD of a completed READ-ONLY preflight — the Decision 126 file DID NOT EXIST BEFORE THE PREFLIGHT AND DID NOT AUTHORIZE IT. THE VERDICT IS NOT_READY_IMPLEMENTATION_GAP AND ITS SHAPE MUST BE READ WHOLE: THIS IS NOT A LIVE-STATE FAILURE, EVERY LIVE-STATE PREDICATE PASSED, COMPLETE-SOURCE EXECUTION DID NOT OCCUR, AND COMPLETE-SOURCE EXECUTION REMAINS UNAUTHORIZED. MEASURED LIVE STATE (D126-R2): entry free 137,633,968,128 bytes = 128.1816 GiB and exit free 136,576,831,488 bytes = 127.1971 GiB, BOTH CLEARING the formal D124-R5 gate of 112,742,891,520 bytes = 105 GiB AND the D125-R6 owner preference of 118,111,600,640 bytes = 110 GiB, with exit standing 23,833,939,968 bytes = 22.1971 GiB above the gate; the D125 shortfall of 7.6760 GiB is CLOSED BY MEASUREMENT from ordinary non-project storage with NO FURTHER DISCLOSURE DRIFT EVIDENCE DELETED; 0 GiB of material stale pytest scratch remained; a publication-time re-read returned 136,511,447,040 bytes = 127.1362 GiB, about 65 MB below the exit figure, and ALL THREE READINGS CLEAR BOTH LEVELS. FROZEN SOURCE VERIFIED: sec_bulk_submissions-c85744be921b0dc5.zip, 1,556,847,020 bytes, SHA-256 c85744be921b0dc5be4e3c7dd44552fc0f57d354d61df38cd92a13926982b82f, 985,835 raw ZIP entries, 985,834 governed JSON members, exactly one non-JSON placeholder.txt. OPERATIONAL CATALOG: file SHA-256 57e36a788dc8e03ea4d1a4c722418de4c4244d73590c6643feace93c80af2ded, logical digest 5c823d216957c0035babd4956f9d9e0c3c0b8ea54455231436a514191c6ad306, observation-set digest b1122bb9fbb084411ce3cb3b7d192c7874c8969aadbb29f6ca313543b8e533be, MIGRATION HEAD 15, 76 PLAN SOURCES, ALL PARSER STATES not_started, AND ZERO E0 PARSER/ACCESSION/RECORD MATERIALIZATION RESIDUE — the path would START FROM A CLEAN STATE, NOT RESUME INTO ONE. THE SOURCE-POINTER RECONCILIATION IS DISCHARGED (D126-R4), closing what Decision 124 section 4 opened and Decision 125 section 12 carried forward: runtime authority is the SINGLE-VALUED census_plan_sources.observation_id 09fdacf651ba4d0a80f5d2ab4e36f4a3 plus its 985,834 governed member rows, DECISION 059 REMAINS HISTORICALLY CORRECT AND IS NOT AMENDED, and the object identifier is recorded as the FIRST SIXTEEN HEX CHARACTERS OF ITS OWN SHA-256 so pointer and bytes verify each other; ONE NONBLOCKING DOCUMENTATION LIMITATION IS RECORDED RATHER THAN REPAIRED — the controlling Aug-11 observation carries NO EXPLICIT supersedes_observation_id EDGE to the Aug-07 one, which affects NO RUNTIME CORRECTNESS and AUTHORIZES NO CATALOG WRITE. THE D125 SECTION 10 RESIDUE CHECK WAS PERFORMED (D126-R3): m3_3_e0_offline_parse_v1 and m3_3_e0_offline_parse_v2 remain present and CONSUMED, NO v3 EXISTS, both are STRUCTURALLY IRRELEVANT to the canary path, and the lease is RELEASED — lease_id 3a37a7ee0e0c4496a4fba845d4d5d2a1, writer_pid 68482 DEAD, released_at_utc 2026-08-18T01:44:33.497032Z, file SHA-256 8c0f251f03c3c113dea7ab59f7c542a02721d6246eefd9b35cf21227019db29b, advisory lock AVAILABLE — so THE FORMER STALE-HELD CONDITION IS ABSENT, NO RECOVERY AUTHORITY OR ACTION IS REQUIRED, AND THE FILE MUST NOT BE MODIFIED. THE SOLE D126 BLOCKER WAS ONE MISSING LINE OF PRODUCTION CODE: no >= 30 GiB admission predicate stood between F1 count_persisted_accession_resolutions(...) and F2 materialize_census_associations(...), and Decision 126 section 7 ruled that NO EXTERNAL SAMPLER CAN SUPPLY IT because no enforceable pause exists at the boundary, ledger state does not distinguish F1 from impending F2, an external process can signal but cannot decline admission atomically, and a sampling race survives any cadence. D127 NOW IMPLEMENTS EXACTLY THAT GUARD AND NOTHING ELSE: PRE_F2_MINIMUM_FREE_BYTES = 30 * 1024**3 = 32,212,254,720 bytes exported in __all__, _require_pre_f2_free_space() measuring shutil.disk_usage(...).free on the disposable world's own volume with a STRICT < so the floor itself admits, raising SingleSourceCanaryError naming the actual free bytes, the required minimum, and that F2 WAS REFUSED BEFORE ITS SINGLE TRANSACTION OPENED and naming no path, and the call site placed BETWEEN F1 AND F2 in _materialize with the Decision 094 section 6.4 ordering UNCHANGED — 57 added lines in one file, nothing removed or modified, plus one new test module of 6 PASSING PROOFS whose NON-VACUITY IS PROVED BY TWO MUTATIONS (deleting the guard call gives 2 failures; weakening < to <= gives 1), both applied, measured, and reverted. D127 IS READY FOR INDEPENDENT REVIEW AND IS NOT OWNER-ACCEPTED, AND NO OWNER-ACCEPTANCE TOKEN WAS EMITTED. ONE CONSEQUENCE IS STATED RATHER THAN SMOOTHED (Decision 127 section 5): the guard applies to EVERY run of the path including the synthetic D116/D119 suites, so on a machine with less than 30 GiB free those suites would refuse at the admission gate — making the floor injectable is SCOPE EXPANSION and a test-overridable safety floor is weaker than D124-R5 asked for. MIGRATION HEAD REMAINS 0015 and 0016 IS ABSENT AND UNAPPLIED. ALL THREE ACTIVATION CONSTANTS REMAIN None. E0-v3 REMAINS UNAUTHORIZED. NO COMPLETE-SOURCE RUN IS AUTHORIZED. D126 AND D127 SUPERSEDE NOTHING — Decisions 121 through 125 stand as written and the D124-R5 gates carry forward intact, INCLUDING the >= 30 GiB pre-F2 gate, which D126 recorded as UNENFORCED BY CODE rather than as relaxed and which D127 now enforces.
ACTIVE_BLOCKER: INDEPENDENT REVIEW AND OWNER ACCEPTANCE OF THE D127 GUARD — AND NO REAL EXECUTION AUTHORIZATION OF ANY KIND EXISTS IN THIS REPOSITORY. THE CAPACITY BLOCKER IS GONE: D126 measured 128.1816 GiB entry and 127.1971 GiB exit against the 105 GiB gate, so the D125 7.6760 GiB gap is CLOSED BY MEASUREMENT, NOT BY PROJECTION, and D126-R2 rules that NO ADDITIONAL STORAGE CLEANUP IS REQUIRED NOW. THE IMPLEMENTATION BLOCKER IS ALSO GONE AS AN IMPLEMENTATION MATTER: the missing >= 30 GiB pre-F2 admission predicate that D126 named as the SOLE blocker is now written, proved by 6 focused tests, and proved NON-VACUOUS by two reverted mutations. WHAT BLOCKS NOW IS ACCEPTANCE, NOT ENGINEERING. D127 CARRIES THE TOKEN M3_3_D127_PRE_F2_GUARD_IMPLEMENTATION_READY_FOR_INDEPENDENT_REVIEW AND IS EXPRESSLY NOT OWNER-ACCEPTED; INDEPENDENT REVIEW COMES FIRST, THEN OWNER ACCEPTANCE. EVEN THEN, ACCEPTANCE ALONE STILL DOES NOT OPEN THE RUN: Decision 126 section 10 (D126-R8) requires BOTH a REGENERATED RUN IDENTITY AND COMMAND — the proposed m3_3_d126_complete_first_source_v1 and its command are EXPRESSLY NOT FROZEN, because D127 changed the published code baseline — AND A NEW FINAL LIVE PREFLIGHT. CLOSING THE D126 SECTION 7 GAP IS NOT OPENING THE RUN: it removes the reason the run could not be authorized, which is a different thing from authorizing it. Nothing in this repository is an execution authorization: not this file, not a passing gate, not a commit, not a push, not the D126 record, and not the D127 implementation. D118 R23 still binds. The complete-source canary, the full-population F2 rerun, the D117 retry, the three-source canary, and the real replay proof remain NOT RUN and NOT AUTHORIZED. All three execution authorities are None, migration 0016 is unapplied and the operational catalog stays at head 0015, no E0-v3 namespace exists, and both tracked network switches stay false at request ceiling 0.
IMPLEMENTATION_AUTHORIZATION: EXERCISED FOR ONE GOVERNANCE PUBLICATION AND ONE MINIMAL BOUNDED IMPLEMENTATION — THE OWNER'S 2026-08-20 DECISION 126 COMPLETE FIRST-SOURCE FINAL LIVE PREFLIGHT OWNER-ACCEPTANCE / GOVERNANCE PUBLICATION INSTRUCTION, TOKEN M3_3_D126_COMPLETE_SOURCE_PREFLIGHT_OWNER_ACCEPTED, AUTHORIZED THE DOCUMENTATION-ONLY D126 PUBLICATION by expressly directing the creation or update of Decision 126, the decision registry, the decision index, and STATUS; AND THE OWNER'S SAME-DAY CONTINUATION INSTRUMENT, TOKEN M3_3_D126_PUBLICATION_AND_D127_PRE_F2_GUARD_IMPLEMENTATION_AUTHORIZED, AUTHORIZED THE D127 PRE-F2 GUARD IMPLEMENTATION IN THIS SAME SESSION BEFORE THE FINAL COMMIT, expressly directing the constant, the guard, the three focused proofs, the D127 durable record, and the registry/index/STATUS/change-impact updates convention requires. THE D127 SCOPE WAS EXACTLY WHAT THAT INSTRUMENT NAMED AND NOTHING MORE: src/disclosure_drift/m3/single_source_canary.py gains PRE_F2_MINIMUM_FREE_BYTES = 30 * 1024**3, the _require_pre_f2_free_space() guard, and one call site between F1 and F2 — 57 added lines, 0 removed, 0 modified — plus the new tests/unit/test_d127_pre_f2_admission_guard.py. THE INSTRUMENT'S PROHIBITIONS WERE HONOURED: NO 105 GiB LAUNCH GATE AND NO CONTINUOUS 10 GiB MONITOR IN PRODUCTION CODE, create_world UNTOUCHED, TEMP HANDLING UNTOUCHED, F1/F2 ALGORITHMS UNTOUCHED, WAL/CHECKPOINT/CACHE/synchronous UNTOUCHED, SCHEMAS AND MIGRATIONS UNTOUCHED, E0 AND THE AUTHORITY CONSTANTS UNTOUCHED, NETWORK NOT ENABLED, AND NO CANARY RUN. The touched paths are Docs/Decisions/decision_126_m3_3_complete_first_source_final_preflight.md (new), Docs/Decisions/decision_127_m3_3_pre_f2_admission_guard.md (new), two Docs/Decisions/decision_registry.md index rows, sixteen Docs/decision_index.md navigation rows, one new Docs/change_impact_map.md section, this file, src/disclosure_drift/m3/single_source_canary.py, and tests/unit/test_d127_pre_f2_admission_guard.py (new). THE CHANGE-IMPACT MAP IS UPDATED THIS TIME — UNLIKE D117, D118 AND D120 THROUGH D126, WHICH CHANGED NO CODE SURFACE — BECAUSE D127 DOES CHANGE ONE. src/disclosure_drift/m3/capacity_plan.py IS UNTOUCHED and no Decision 113 capacity constant moved. THE D126 PREFLIGHT ITSELF WAS PERFORMED BEFORE AND OUTSIDE THIS PUBLICATION, under the GPT-5.6 Sol owner instrument issued at the time; THIS SESSION EXECUTED NO GOVERNED OPERATION: no F1, no F2, no canary, no disposable world, no complete-source run, no E0, no network, no migration 0016, no governed SQLite writer, and no catalog, source, or evidence mutation. Read-only verification of the published facts was performed and is reported: git entry state, migration head, the three authority constants, both network switches, statvfs free space, the frozen source's byte length and SHA-256 (both matched EXACTLY), the operational catalog's file SHA-256 read as BYTES WITHOUT OPENING THE DATABASE (matched EXACTLY, with a 0-byte WAL), the lease file's SHA-256 and every recorded field (all matched EXACTLY), writer_pid 68482 confirmed DEAD, and the E0 namespace inventory (v1 and v2 present, NO v3). THE CATALOG'S LOGICAL AND OBSERVATION-SET DIGESTS AND ITS PLAN/PARSER-STATE COUNTS ARE RECORDED AS OWNER-ACCEPTED D126 MEASUREMENTS AND WERE NOT RE-MEASURED HERE, because re-measuring them requires opening the database and this publication opened none. NO OWNER-ACCEPTANCE TOKEN WAS EMITTED BY THIS SESSION.
NEXT_AUTHORIZED_ACTION: A FRESH INDEPENDENT REVIEW OF THE D127 PRE-F2 ADMISSION GUARD, FOLLOWED BY OWNER ACCEPTANCE, FOLLOWED BY A REGENERATED RUN IDENTITY AND A NEW FINAL LIVE M3.3 COMPLETE-SOURCE PREFLIGHT. NONE OF THESE IS EXECUTED AND NONE IS AUTHORIZED BY ANYTHING WRITTEN HERE — this entry records OWNER SEQUENCING ONLY. D127 carries M3_3_D127_PRE_F2_GUARD_IMPLEMENTATION_READY_FOR_INDEPENDENT_REVIEW and is NOT owner-accepted; the review comes first. EVEN AFTER ACCEPTANCE, DECISION 126 SECTION 10 (D126-R8) REQUIRES BOTH: the proposed run ID m3_3_d126_complete_first_source_v1 AND ITS COMMAND ARE NOT FROZEN and must be REGENERATED because D127 changed the published code baseline, AND a NEW FINAL LIVE PREFLIGHT must run and pass. THE FOUR NON-CODE D124-R5 REQUIREMENTS REMAIN THE LAUNCH WRAPPER'S TO ENFORCE (D126-R5): start >= 105 GiB, the continuous 10 GiB floor through interruptible phases, no VACUUM, and SQLITE_TMPDIR pointed at the SIBLING directory <work-root>/<run-id>__sqlite_tmp/ rather than <world>/tmp, BECAUSE PRECREATING <world>/tmp VIOLATES CREATE-ONCE WORLD SEMANTICS. A FUTURE RUN ALSO REQUIRES AN EXPLICIT PER-PROCESS NO-SLEEP ASSERTION — caffeinate -i -m -s — against the current sleep=1/disksleep=10 settings, AND PERSISTENT pmset SETTINGS MUST NOT BE ALTERED. THERE IS NO ARBITRARY WALL-CLOCK KILL CEILING (D126-R7): the ~15 h / ~24 h / ~34 h+ figures are a PLANNING ENVELOPE ONLY, resource and progress monitoring controls, and because the run is NOT RESUMABLE elapsed time alone is NOT a valid kill criterion. COMPLETE SOURCE IS NOT AUTHORIZED AND E0 IS NOT AUTHORIZED. F1, F2, the full-population F2 rerun, the D117 retry, the three-source canary, the real replay proof, migration 0016, network, and acquisition all remain unauthorized. NO FURTHER DISCLOSURE DRIFT EVIDENCE MAY BE DELETED FOR CAPACITY (D125-R3), the released lease MUST NOT BE MODIFIED (D126-R3), the absent supersession edge AUTHORIZES NO CATALOG WRITE (D126-R4), the SSK SSD REMAINS COLD/ARCHIVE ONLY WITH NO REFORMAT (D125-R4), and Plan B remains NOT APPROVED. Do not emit an owner-acceptance token — this session may publish an accepted ruling and may not accept anything.


> **CONTROLLING CURRENT POSITION, 2026-08-20 (LATEST) — the
